...
{
    public partial class ucEquipmentInspectInfoPopup : UserControl
    {
        ...
        private ucEquipmentInspectInfoPopupEntity _pucEquipmentInspectInfoPopupEntity = null;
        ...
        public ucEquipmentInspectInfoPopup()
        {
            InitializeComponent();
        ...
        private void InitializeSetting()
        {
            ...
            _pucEquipmentInspectInfoPopupEntity = new ucEquipmentInspectInfoPopupEntity();
            _pucEquipmentInspectInfoPopupEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
            ...
            _luTDATE.DateTime = DateTime.Today;
            ...
            _pucEquipmentInspectInfoPopupEntity.CRUD = "R";
        ...
        {
            _pucEquipmentInspectInfoPopupEntity.CRUD = "R"; // 발주내역 있게 조회하는 경우, R로 보냄
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                if (_pucEquipmentInspectInfoPopupEntity.CRUD == "") _dtList.Rows.Clear();
        ...
        #region ● 저장등록 - _ucbtPLAN_SAVE_Click(object sender, EventArgs e)
            ...
            try
            {
                ...
                if (_gdMAIN_VIEW.RowCount > 0)
                {
                    for (int i = 0; i < _gdMAIN_VIEW.RowCount; i++)
                    {
                        ...
                        if (_luTDATE.DateTime > lastDay)
                        {
                            // 이번 달, 지난 달 양식만 추가할 수 있습니다.
                            CoFAS_DevExpressManager.ShowInformationMessage("今月,先月の様式だけを追加することができます。");
                            return;
                        }

                        if (_gdMAIN_VIEW.GetRowCellValue(i, "EQUIPMENT_NAME").ToString() == "")
                        {
                            // 설비 정보가 없습니다.
                            CoFAS_DevExpressManager.ShowInformationMessage("設備情報がありません。");
                            return;
                        }

                        if (_gdMAIN_VIEW.GetRowCellValue(i, "CRUD").ToString() == "Y")
                        {
                            num = num + Convert.ToInt32(_gdMAIN_VIEW.GetRowCellValue(i, "EQUIPMENT_COUNT").ToString());

                            if (num > 12)
                            {
                                // 설비항목이 양식 기준을 넘겼습니다.
                                CoFAS_DevExpressManager.ShowInformationMessage("設備項目が様式基準を超えました。");
                                return;
                            }
                            else if (num < 1)
                            {
                                // 설비 항목이 없습니다.설비에 설비항목을 매핑해 주세요.
                                CoFAS_DevExpressManager.ShowInformationMessage("設備項目がありません。設備に設備項目をマッピングしてください。");
                                return;
                            }
                        }
                    }

                    _pucEquipmentInspectInfoPopupEntity.CRUD = _pCRUD;

                    _pucEquipmentInspectInfoPopupEntity.DATE = DateTime.Parse(_luTDATE.DateTime.ToString()).ToString("yyyyMMdd");
                ...
                {
                    // 설비정보를 확인해주세요.
                    CoFAS_DevExpressManager.ShowInformationMessage("本気で情報を示します。");
                    return;
                }
            }
            catch (ExceptionManager pExceptionManager)
        ...
        private void InputData_Save(DataTable pDataTable)
        {
            try
            {
                ...
                {
                    //체크된것만 저장
                    if (pDataTable.Rows[a]["CRUD"].ToString() == "Y")
                    {
                        chk = "Y";
                    }
                }
                ...
                {
                    bool isError = false;
                    _pucEquipmentInspectInfoPopupEntity.USER_CODE = _pUSER_CODE;

                    isError = new ucEquipmentInspectInfoPopupBusiness().ucEquipmentInspectInfoPopup_Save(_pucEquipmentInspectInfoPopupEntity, pDataTable);
