...
{
    public partial class ucMaterialOrderInfoPopup_T01 : UserControl
    {
        ...
        private MaterialOrderInfoPopup_T01Entity _pMaterialOrderInfoPopup_T01Entity = null;
        ...
        public ucMaterialOrderInfoPopup_T01()
        {
            InitializeComponent();
        ...
        private void InitializeSetting()
        {
            ...
            _pMaterialOrderInfoPopup_T01Entity = new MaterialOrderInfoPopup_T01Entity();
            _pMaterialOrderInfoPopup_T01Entity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
        ...
        private void _ucbtSELECT_Click(object sender, EventArgs e)
        {
            ...
            {
                _pCRUD = "R"; // 발주내역 있게 조회하는 경우, R로 보냄
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                string pType = _pARRAY[1].ToString();
                ...
                if (_pCRUD == "R")
                {
                    FromDATE = _luTORDER_DATE.FromDateTime.ToString("yyyyMMdd");
                    ToDATE = _luTORDER_DATE.ToDateTime.ToString("yyyyMMdd");

                    strCODE = "";
                    strNAME = _luPART_NAME.Text.ToString();   //자재명
                    strVNAME = _luVEND_NAME.Text.ToString();  //거래처명
                }
                else
                {
                    strCODE = _luTPART_CODE.Text.ToString();  //자재코드
                    strNAME = _luTPART_NAME.Text.ToString();  //자재명
                    strVNAME = "";
                }
                
                _dtList = new MaterialOrderInfoPopup_T01Business().MaterialOrderInfoPopup_T01_Return(_pCRUD, FromDATE, ToDATE, strCODE, strNAME, strVNAME, pType).Tables[0];
        ...
        private void _btSELECT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                frmCommonPopup.ARRAY = new object[2] { service_name, _luTPART_NAME.Text }; //넘기는 파라메터 에 따라 설정, 제품/조회할때 품목명을 던지기
        ...
        private void _ucbtPLAN_SAVE_Click(object sender, EventArgs e)
        {
            try
            {
                if (_gdMAIN_VIEW.RowCount > 0)
                {
                    ...
                    {
                        if (_gdMAIN_VIEW.GetRowCellValue(i, "PART_NAME").ToString() == "")
                        {
                            CoFAS_DevExpressManager.ShowInformationMessage("MATERIAL DATA NOT INPUT");
                            return;
                        }
                    }

                    _pMaterialOrderInfoPopup_T01Entity.CRUD = _pCRUD;
        ...
        private void InputData_Save(DataTable pDataTable)
        {
            try
            {
                ...
                {
                    bool isError = false;
                    _pMaterialOrderInfoPopup_T01Entity.USER_CODE = _pUSER_CODE;

                    if (_pARRAY[1].ToString() == "1004")
                    {
                        isError = new MaterialOrderInfoPopup_T01Business().MaterialOrderInfoPopup_T01_Save(_pMaterialOrderInfoPopup_T01Entity, pDataTable);
                    }
                    else
                    {
                        isError = new MaterialOrderInfoPopup_T01Business().OrderInfoPopup_T01_Second_Save(_pMaterialOrderInfoPopup_T01Entity, pDataTable);
                    }
