...
namespace CoFAS_MES.CORE.UserControls
{
    /*  
        등록일 : 2018.06.21 
        수정일 : 2018.06.21
        기존 : ucPartCodeInfoPopup
        내용 : 전체품목을 조회하는 팝업
        수정자 : 김기환
    */

    public partial class ucVendCodeInfoPopup_T01 : UserControl
    {
        ...
        public ucVendCodeInfoPopup_T01()
        {
        ...
        private void InitializeSetting()
        {
            ...
            {
                _luPART_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "PART_TYPE_PRODUCT", "", "", "").Tables[0], 0, 0, "");
            }
            else if  (DBManager.InitDatabaseName == "coever_mes_biocerra")
            {
                _luPART_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "PART_TYPE_PRODUCT_02", "", "", "").Tables[0], 0, 0, "");
            }
            else if ((DBManager.InitDatabaseName == "coever_mes_seongyojeongmil") || 
                (DBManager.InitDatabaseName == "coever_mes_handoteck") || (DBManager.InitDatabaseName == "coever_mes_kdsystem") || (DBManager.InitDatabaseName == "coever_mes_yeonjuneng"))
            {
                _luPART_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "PART_TYPE", "", "", "").Tables[0], 0, 0, "");
            }
            else
            {
