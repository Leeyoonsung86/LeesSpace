...
{
    public partial class ucWorkResultPopUp : UserControl
    {
        #region ○ 기본 전역변수 영역
        ...
        private ucWorkResultPopup_Entity _pucWorkResultPopup_Entity = null;

        private string qWORK_ORDERID = string.Empty;
        private string qPART_CODE = string.Empty;
        private string qPART_NAME = string.Empty;
        private string qPROCESS_CODE1 = string.Empty;
        private string qPROCESS_NAME1 = string.Empty;
        private string qWORKCENTER_CODE = string.Empty;
        private string qWORKCENTER_NAME = string.Empty;
        ...
        #region ○ 폼 로드 이벤트
        private void ucWorkResultPopUp_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                _pucWorkResultPopup_Entity = new ucWorkResultPopup_Entity();
                _pucWorkResultPopup_Entity.CORP_CODE = _pCORP_CODE;
                _pucWorkResultPopup_Entity.USER_CODE = _pUSER_CODE;
                _pucWorkResultPopup_Entity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;

                qWORK_ORDERID = _pARRAY_CODE[0].ToString();
                qPART_CODE = _pARRAY_CODE[1].ToString();
                qPART_NAME = _pARRAY_CODE[2].ToString();

                qPROCESS_CODE1 = _pARRAY_CODE[3].ToString();//5
                qPROCESS_NAME1 = _pARRAY_CODE[4].ToString();//6
                qWORKCENTER_CODE = _pARRAY_CODE[5].ToString(); //3
                qWORKCENTER_NAME = _pARRAY_CODE[6].ToString(); //4
        ...
        private void InitializeControl()
        {
            try
            {
                _luRESULT_GUBN.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "COMMON_CODE", "PO01", "", "").Tables[0], 0, 0, "", false);
                //_luRESULT_GUBN.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "RESULT_GUBN_OWELL", "", "", "").Tables[0], 0, 0, "", false);
                ...
                _luPROCESS_CODE1.Text = qPROCESS_CODE1;
                _luPROCESS_NAME1.Text = qPROCESS_NAME1;
                _luWORKCENTER_NAME.Text = qWORKCENTER_NAME;
                _luWORKCENTER_CODE.Text = qWORKCENTER_CODE;
        ...
        private void _luRESULT_GUBN_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string pResult_Gubn = _luRESULT_GUBN.GetValue();
        ...
        private void _ucbtSAVE_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    _pucWorkResultPopup_Entity.RESOURCE_CODE = _luWORKCENTER_CODE.Text.ToString(); //_luPROCESS_CODE.GetValue(); //_luWORKCENTER_CODE.Text.ToString();
                    _pucWorkResultPopup_Entity.COLLECTION_DATE = DateTime.Now.ToString("yyyyMMddHHmmss");
                    _pucWorkResultPopup_Entity.PROPERTY_VALUE = _luWORKORDERID.Text;

                    switch (_luRESULT_GUBN.GetValue().ToString())
                    {
                        case "CD501001":
                            _pucWorkResultPopup_Entity.CONDITION_CODE = _luRESULT_GUBN.GetValue();
                            _pucWorkResultPopup_Entity.COLLECTION_VALUE = _luRESULT_QTY.Text.Replace(",", "");
                            //_pucWorkResultPopup_Entity.CONDITION_CODE = "CD501001";
                            break;
                        case "CD501002":
                            _pucWorkResultPopup_Entity.CONDITION_CODE = _luBAD_CODE.GetValue();
                            _pucWorkResultPopup_Entity.COLLECTION_VALUE = _luRESULT_QTY.Text.Replace(",", "");
                            break;
                        case "CD501003":
                            _pucWorkResultPopup_Entity.CONDITION_CODE = _luSTATUS_CODE.GetValue();
                            ...
                            _pucWorkResultPopup_Entity.COLLECTION_VALUE = pTimeSpan.TotalSeconds.ToString(); // Math.Floor(pTimeSpan.TotalSeconds).ToString(); // _luSTATUS_TIME.Text.Replace("-", "").Replace(" / ", "").Replace(":", "");
                            break;
                    }
                    ...
                    bool isError = false;
                    _pucWorkResultPopup_Entity.OPTION_CODE = "";
                    _pucWorkResultPopup_Entity.COLLECTION_VALUE_STR = "";
                    isError = new ucWorkResultPopupBusiness().ucWorkResultPopup_Save(_pucWorkResultPopup_Entity);
                    ...
                    if (!isError)
                    {
                        ...
                        if(_luRESULT_GUBN.GetValue().ToString() == "CD501003" && _pucWorkResultPopup_Entity.CONDITION_CODE == "ES011003")
                        {
                            _pucWorkResultPopup_Entity.CRUD = "U";
                            _pucWorkResultPopup_Entity.PRODUCTION_ORDER_ID = _pucWorkResultPopup_Entity.PROPERTY_VALUE;
                            _pucWorkResultPopup_Entity.COMPLETE_YN = "Y";

                            isError = new ucWorkResultPopupBusiness().usWorkResultPopup_Save_01(_pucWorkResultPopup_Entity);
