...
{
    public partial class frmPOPEquipmentCheck : frmBaseSingle
    {
        ...
        public frmPOPEquipmentCheck(UserEntity pUserEntity, string equipment_code)
        {
            InitializeComponent();
            
            _pUserEntity = pUserEntity;
            _pTerminal_code = equipment_code;
            Activated += new EventHandler(Form_Activated);
            FormClosing += new FormClosingEventHandler(Form_Closing);
            FormClosed += new FormClosedEventHandler(Form_FormClosed);
            Load += new EventHandler(Form_Load);

            lbl_equpmentName.Text = "( " + equipment_code + ")";
            lbl_nowdate.Text = DateTime.Now.Date.ToShortDateString();
        }
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new POPEquipmentCheckBusiness().POPEquipmentCheck_Info(_pPOPCheckEntity, lbl_nowdate.Text);
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                //gv.GetFocusedRowCellValue(strName).ToString()가 

                if(gv.GetFocusedRowCellValue("CODE4").ToString() == "POPUP/OX")
                {
                    frmPOPOX frm = new frmPOPOX();
                    frm.titleName = gv.GetFocusedRowCellValue("CODE1").ToString() + " - " + gv.GetFocusedRowCellValue("CODE2").ToString() + " - " + gv.GetFocusedRowCellValue("CHECK_STANDARD").ToString();

                    if(frm.ShowDialog() == DialogResult.OK)
                    {
                        //gv.SetFocusedRowCellValue("CODE5", frm.ReturnValue1);
                        gv.SetRowCellValue(gv.FocusedRowHandle, "CODE5", frm.ReturnValue1);
                        gv.SetRowCellValue(gv.FocusedRowHandle, "CODE8", frm.ReturnValue2);
                    }
                }
                else if(gv.GetFocusedRowCellValue("CODE4").ToString() == "POPUP/DP")
                {
                    frmPOPDP frm = new frmPOPDP();
                    frm.titleName = gv.GetFocusedRowCellValue("CODE1").ToString() + " - " + gv.GetFocusedRowCellValue("CODE2").ToString() + " - " + gv.GetFocusedRowCellValue("CHECK_STANDARD").ToString();
                ...
                else if (gv.GetFocusedRowCellValue("CODE4").ToString() == "POPUP/KP")
                {
                    //frmPOPDP frm = new frmPOPDP();
                    //frm.titleName = gv.GetFocusedRowCellValue("CODE1").ToString() + " - " + gv.GetFocusedRowCellValue("CODE2").ToString() + " - " + gv.GetFocusedRowCellValue("CHECK_STANDARD").ToString();
                    ...
                    {
                        gv.SetRowCellValue(gv.FocusedRowHandle, "CODE5", frmResult.ReturnValue1+"%");
                    }
                }
        ...
        private void _ucbtCANCEL_Click(object sender, EventArgs e)
        {
            ...
            for(int i=0; i < _gdMAIN_VIEW.RowCount; i++)
            {
                if(_gdMAIN_VIEW.GetDataRow(i).ItemArray[6].ToString() == "")
                {
                    tf = true;
                    break;
                }
            ...
            if (tf)
                CoFAS_DevExpressManager.ShowErrorMessage("모든 항목의 점검을 완료해주세요.");
            else
                this.Close();
        }
        #endregion

        private void lbl_nowdate_DoubleClick(object sender, EventArgs e)
        {
            frmPOPDatecheck frm = new frmPOPDatecheck();
            
            if(frm.ShowDialog() == DialogResult.OK)
            {
                lbl_nowdate.Text = frm.ReturnValue1;
            }
