...
{
    public partial class frmPOPFirstMiddleLast : frmBaseSingle
    {
        ...
        private POPFirstMiddleLastEntity _pPOPFirstMiddleLastEntity = null; // 엔티티 생성
        ...
        public frmPOPFirstMiddleLast(UserEntity pUserEntity, string terminal_code, string order_id, string part_code)
        {
            InitializeComponent();
        ...
        private void InitializeControl()
        {
            try
            {
                ...
                _luFIRSTMIDDLELAST_GUBN.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R",_pLANGUAGE_TYPE, "COMMON_CODE", "QC10", "", "").Tables[0], 0, 0, "");
                _luFIRSTMIDDLELAST_GUBN.Font = _pFONT_SETTING2;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                if (_pPOPFirstMiddleLastEntity.CRUD == "") _dtList.Rows.Clear();
        ...
        private void _luFIRSTMIDDLELAST_GUBN_ValueChanged(object sender, EventArgs e)
        {
            MainFind_DisplayData();
        }
        ...
        private void btnCmd_Click(object pSender, EventArgs pEventArgs)
        {
            ...
            switch (sCls)
            {
                ...
                case "30":
                    ...
                    _dtreturn.Columns.Add("제품코드", typeof(string));
                    _dtreturn.Columns.Add("제품명", typeof(string));
                    //_dtreturn.Columns.Add("PROCESS_CODE_MST", typeof(string));
                    _dtreturn.Columns.Add("공정명", typeof(string));
                    //_dtreturn.Columns.Add("PRINT_CODE", typeof(string));
                    //_dtreturn.Columns.Add("PRINT_CMD", typeof(string));
                    ...
                    //_dtreturn.Columns.Add("PRODUCTION_STATUS", typeof(string));

                    _dtreturn.Rows.Add(new Object[] {
                        _gdMAIN_VIEW.GetFocusedRowCellValue("작업지시번호").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("지시일자").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("제품코드").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("제품명").ToString(),
                        //_gdMAIN_VIEW.GetFocusedRowCellValue("PROCESS_CODE_MST").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("공정명").ToString(),
                        //_gdMAIN_VIEW.GetFocusedRowCellValue("PRINT_CODE").ToString(),
                        //_gdMAIN_VIEW.GetFocusedRowCellValue("PRINT_CMD").ToString(),
                        //_gdMAIN_VIEW.GetFocusedRowCellValue("VEND_PART_CODE").ToString(),
                        (int)Convert.ToDouble(_gdMAIN_VIEW.GetFocusedRowCellValue("목표").ToString()),
                        (int)Convert.ToDouble(_gdMAIN_VIEW.GetFocusedRowCellValue("양품").ToString()),
                        (int)Convert.ToDouble(_gdMAIN_VIEW.GetFocusedRowCellValue("불량").ToString()),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("단위").ToString(),
                        //_gdMAIN_VIEW.GetFocusedRowCellValue("COMPLETE_YN").ToString(),
                        //_gdMAIN_VIEW.GetFocusedRowCellValue("PRODUCTION_STATUS").ToString()                        
