...
{
    public partial class frmPOPMain_PRODUCTION : frmBasePOP
    {
        ...
        private DataTable _dtList_print2 = null; //조회 데이터 리스트 (바코드 저장)
        ...
        public frmPOPMain_PRODUCTION(UserEntity pUserEntity, string termial_code)
        {
            InitializeComponent();
    ...
    */
        ...
        private void _ucbtPRINT_REG_Click_1(object sender, EventArgs e)
        {

            SetHardware();

            frmPOPLabelPrint frmPrint = new frmPOPLabelPrint(_pUserEntity);
            frmPrint.ShowDialog();
            if (frmPrint.dtReturn == null)
            {
                frmPrint.Dispose();
                return;
            }
            _pP_Code = frmPrint._pPartCode;
            _pP_Name = frmPrint._pPartName;
            _pP_QTY = frmPrint._pPrintQTY;
