...
{
    public partial class frmPOPMain_PRODUCT_COSMETICS : frmBasePOP
    {
        ...
        public frmPOPMain_PRODUCT_COSMETICS(UserEntity pUserEntity)
        {
            InitializeComponent();
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                if (_pFirstYN)
                {
                    ...
                    {
                        _ucbtORDER_REQUEST.Visible = false;
                        _ucbtORDERDOC_REG.Visible = false;
                    }
                    _pFirstYN = false;
                }
                GetBarcodeComPort();
            ...
            public void Size_reset()
        {
            ...
            FontResize(_luTPROCESS_CODE_MST);
        ...
        {
            //작업지시
            frmPOPOrder.PROCESS_MST_CODE_COSMETICS = "PC02";// "ALL";
            ...
            if (frmOr.dtReturn != null && frmOr.dtReturn.Rows.Count > 0)
            {
                ...
                _pPOPSelect_INSPECT_COSMETICSEntity.ORDER_FILE = frmOr.dtReturn.Rows[0]["ORDER_FILE"].ToString();
                _pPOPSelect_INSPECT_COSMETICSEntity.REQUEST_FILE = frmOr.dtReturn.Rows[0]["REQUEST_FILE"].ToString();
                _pPOPSelect_INSPECT_COSMETICSEntity.PRODUCTION_STATUS_PACKING = frmOr.dtReturn.Rows[0]["PRODUCTION_STATUS_PACKING"].ToString();


                _pPOPSelect_INSPECT_COSMETICSEntity.PROCESS_CODE_MST = frmOr.dtReturn.Rows[0]["PROCESS_CODE_MST"].ToString();// _pUserEntity.PROCESS_CODE;
                _luTPROCESS_CODE_MST.Text = frmOr.dtReturn.Rows[0]["PROCESS_NAME"].ToString();
                //제조일경우에는 요청서 안보이게
                if (_pPOPSelect_INSPECT_COSMETICSEntity.PROCESS_CODE_MST == "PT040001" || _pPOPSelect_INSPECT_COSMETICSEntity.PROCESS_CODE_MST == "PC01")
                    _ucbtORDER_REQUEST.Enabled = false;
                else
                    _ucbtORDER_REQUEST.Enabled = true;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pPOPProductionOrderEntity.CRUD == ""))
                {
                    ...
                    _pPOPSelect_INSPECT_COSMETICSEntity.ORDER_FILE = _dtList.Rows[0]["ORDER_FILE"].ToString();
                    _pPOPSelect_INSPECT_COSMETICSEntity.REQUEST_FILE = _dtList.Rows[0]["REQUEST_FILE"].ToString();
                    _pPOPSelect_INSPECT_COSMETICSEntity.PRODUCTION_STATUS_PACKING = _dtList.Rows[0]["PRODUCTION_STATUS_PACKING"].ToString();
        ...
        private void _ucbtORDERDOC_REG_Click(object sender, EventArgs e)
        {
            try
            {
                if (_pPOPSelect_INSPECT_COSMETICSEntity.PRODUCTION_ORDER_ID != "" && _pPOPSelect_INSPECT_COSMETICSEntity.PRODUCTION_ORDER_ID != null)
                {
                    ...
                    CoFAS_MES.POP.Popup.frmPOPSelect_WorkOrderDoc_COSMETICS.pCOM_PORT = COM_PORT;

                    CoFAS_MES.POP.Popup.frmPOPSelect_WorkOrderDoc_COSMETICS.ARRAY = new object[5] { "ucWorkOrderDocRegPopup", _pPOPSelect_INSPECT_COSMETICSEntity.PROCESS_CODE_MST, _pPOPSelect_INSPECT_COSMETICSEntity.PRODUCTION_ORDER_ID, _pPOPSelect_INSPECT_COSMETICSEntity.ORDER_FILE, _pPOPSelect_INSPECT_COSMETICSEntity.TERMINAL_CODE }; //넘기는 파라메터 에 따라 설정, 제품/조회할때 품목명을 던지기
        ...
        private void _ucbtORDER_REQUEST_Click(object sender, EventArgs e)
        {
            try
            {
                if (_pPOPSelect_INSPECT_COSMETICSEntity.PRODUCTION_ORDER_ID != "" && _pPOPSelect_INSPECT_COSMETICSEntity.PRODUCTION_ORDER_ID != null)
                {
                    ...
                    CoFAS_MES.POP.Popup.frmPOPSelect_WorkRequestDoc_COSMETICS.ARRAY = new object[4] { "ucWorkRequestDocRegPopup", _pPOPSelect_INSPECT_COSMETICSEntity.PROCESS_CODE_MST, _pPOPSelect_INSPECT_COSMETICSEntity.PRODUCTION_ORDER_ID, _pPOPSelect_INSPECT_COSMETICSEntity.REQUEST_FILE }; //넘기는 파라메터 에 따라 설정, 제품/조회할때 품목명을 던지기
                    CoFAS_MES.POP.Popup.frmPOPSelect_WorkRequestDoc_COSMETICS.ARRAY_CODE = new object[3] { "", "", part_type };
