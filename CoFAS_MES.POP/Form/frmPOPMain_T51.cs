...
{
    public partial class frmPOPMain_T51 : frmBasePOP
    {
        ...
        public int _pFONT_SIZE = 15;                     // 시스템 폰트 사이즈 설정 기본 9
        ...
        private POP_MAIN_HWTEntity _pPOP_MAIN_HWTEntity = null; // 엔티티 생성
        ...
        public frmPOPMain_T51(UserEntity pUserEntity)
        {
            InitializeComponent();
            vScrollBar = _gdMAIN.Controls.OfType<DevExpress.XtraEditors.VScrollBar>().FirstOrDefault();
            ...
            _pPOP_MAIN_HWTEntity = new POP_MAIN_HWTEntity();
        ...
        private void Form_Load(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                else
                {
                    ...
                    lbl_worker.Text = frmPOPWorker_T50.dtReturn.Rows[0]["USER_NAME"].ToString();
            ...
            {
                DevExpress.Utils.AppearanceObject.DefaultFont = new Font(_pFONT_TYPE, 15);//화면에 모든 항목 폰트 재설정
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                if (e.Column.FieldName == "WORK_TIME")
                {
                    frmPopupKeypad_T01 frmResult = new frmPopupKeypad_T01();

                    if (frmResult.ShowDialog() == DialogResult.OK)
                    {
                        gv.SetRowCellValue(gv.FocusedRowHandle, "WORK_TIME", frmResult.ReturnValue1);
                       
                        DataTable dt = new POP_MAIN_hwtBusiness().USP_work_state_I10(_gdMAIN_VIEW.GetDataRow(gv.FocusedRowHandle).ItemArray, lbl_project_name.Text, lbl_process_name.Text);                                   
                    }
                }
                else if(e.Column.FieldName == "COMPLETE_YN")
                {
                    frmPOPOX.button_title = "완료 여부 입력";
                    frmPOPOX frmResult = new frmPOPOX();
                    frmResult.titleName = gv.GetFocusedRowCellValue("MIDDLE_CODE").ToString() + " - " + gv.GetFocusedRowCellValue("UNIT_WORK").ToString();

                    if (frmResult.ShowDialog() == DialogResult.OK)
                    {
                     

                        if (gv.GetFocusedRowCellValue("WORK_TIME").ToString() != "")
                        {

                            gv.SetRowCellValue(gv.FocusedRowHandle, "COMPLETE_YN", frmResult.ReturnValue1);
                            gv.SetRowCellValue(gv.FocusedRowHandle, "REMARK", frmResult.ReturnValue2);

                            DataTable dt = new POP_MAIN_hwtBusiness().USP_work_state_I10(_gdMAIN_VIEW.GetDataRow(gv.FocusedRowHandle).ItemArray, lbl_project_name.Text, lbl_process_name.Text);                            
                        }
                        else
                        {
                            CoFAS_DevExpressManager.ShowErrorMessage("작업시간이 입력되지 않았습니다.");
                        }
                    }
                }               
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메인 화면 전역 변수 처리
                _pFONT_SETTING = new Font(_pFONT_TYPE, 15);
                //회사코드
        ...
        private void MainFind_DisplayData(string process, string order_id)
        {
            try
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);

                _pPOP_MAIN_HWTEntity.CRUD = "R";
                _pPOP_MAIN_HWTEntity.EQUIPMENT_CODE = _pTerminal_code;
                //_pPOPCheckEntity.CHECK_CYCLE = _luTCHECK_LIST.GetValue();

                _dtList = new POP_MAIN_hwtBusiness().USP_process_mst_hwt_R10(process, order_id);

                if (_pPOP_MAIN_HWTEntity.CRUD == "") _dtList.Rows.Clear();


                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pPOP_MAIN_HWTEntity.CRUD == ""))
                {
                    CoFAS_DevExpressManager.BindGridControl(_gdMAIN, _gdMAIN_VIEW, _dtList); //데이터 필드에 맞춰 자동 바인딩

                    _gdMAIN_VIEW.RowHeight = 50;
        ...
        private void btnCmd_Click(object pSender, EventArgs pEventArgs)
        {
            ...
            switch (sCls)
            {
                ...
                case "20":
                    ...
                    if (_gdMAIN_VIEW.FocusedRowHandle <= _gdMAIN_VIEW.RowCount - 1)
                    {
                        _gdMAIN_VIEW.FocusedRowHandle = _gdMAIN_VIEW.FocusedRowHandle - 1;
                        Acitve_Row = _gdMAIN_VIEW.FocusedRowHandle;
                        _gdMAIN_VIEW.SelectRow(Acitve_Row);
                    }                    
                    break;               
        ...
        private void _ucbtWORK_ORDER_Click(object sender, EventArgs e)
        {
            ...
            //작업지시
            frmPOPOrder_T51 frmOr = new frmPOPOrder_T51(_pUserEntity);                        
            frmOr.ShowDialog();
            ...
            if (frmOr.dtReturn != null && frmOr.dtReturn.Rows.Count > 0)
            {
                ...
                lbl_project_name.Text = frmOr.dtReturn.Rows[0]["PRODUCTION_ORDER_ID"].ToString();
                lbl_process_name.Text = frmOr.dtReturn.Rows[0]["PROCESS_CODE_MST"].ToString();
                lbl_unit_name.Text = frmOr.dtReturn.Rows[0]["PART_NAME"].ToString();

                MainFind_DisplayData(frmOr.dtReturn.Rows[0]["PROCESS_CODE_MST"].ToString(), lbl_project_name.Text);
            }
        ...
        private void _ucbtCOMPLETE_REG_Click(object sender, EventArgs e)
        {
            ...
            if (CoFAS_DevExpressManager.ShowQuestionMessage("해당 작업지시를 완료 하시겠습니까?") == DialogResult.Yes)
            {
                ...
                {
                    if (_gdMAIN_VIEW.GetDataRow(i).ItemArray[5].ToString() == "")
                    {
                        tf = true;
                        break;
                    }
                ...
                if (tf)
                    CoFAS_DevExpressManager.ShowErrorMessage("모든 항목의 완료여부를 확인 해주세요.");
                else
                {
                   

                    _pPOPWorkResult_T50Entity.CRUD = "U";
                    _pPOPWorkResult_T50Entity.PRODUCTION_ORDER_ID = lbl_project_name.Text;
                    _pPOPWorkResult_T50Entity.PRODUCTION_ORDER_SEQ = lbl_process_name.Text;
                    _pPOPWorkResult_T50Entity.COMPLETE_YN = "Y";

                  
                    new POP_MAIN_hwtBusiness().USP_production_order_I20(_pPOPWorkResult_T50Entity);
        ...
        private void _ucbtIMAGE_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataTable dt = new POPProductImageCommonBusiness().POPProductImage_Info(_pPOPWorkResult_T50Entity);
                ...
                {
                    if (dt.Rows[0]["image_name"] != null)
                    {
                        MemoryStream _ms = new MemoryStream();

                        if (dt != null && dt.Rows.Count > 0)
                        {
                            floorplan(lbl_unit_name.Text);
                        }
                    }
                    else
                    {
        ...
        private void _ucbCANCEL_REG_Click(object sender, EventArgs e)
        {
            ...
            frmPOPCancel_T50 frmCancel = new frmPOPCancel_T50(lbl_project_name.Text, lbl_process_name.Text);
        ...
        public void floorplan(string part)
        {
            string strFTP_PATH = string.Format(@"{0}{1}/%2F/{2}/{3}/", _pFTP_PATH, "ITEM", "ITEM_IMAGE", "ProductInformationRegister");

            var stream = CoFAS_FTPManager.FTPImage(strFTP_PATH, part, _pFTP_ID, _pFTP_PW);
