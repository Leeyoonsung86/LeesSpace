...
{
    public partial class frmPOPOrderBOM : frmBaseSingle
    {
        ...
        public string _Workorder_ID = string.Empty;
        public string _Part_Code = string.Empty;

        public POPSelect_INSPECT_MIXEntity _pPOPSelect_INSPECT_MIXEntity = null; // 엔티티 생성
        ...
        public frmPOPOrderBOM(UserEntity pUserEntity, POPSelect_INSPECT_MIXEntity pPOPSelect_INSPECT_MIXEntity,string _pWorkorder_ID, string _pPart_Code)
        {
            InitializeComponent();
            _pUserEntity = pUserEntity;
            _Workorder_ID = _pWorkorder_ID;
            _Part_Code = _pPart_Code;

            _pPOPSelect_INSPECT_MIXEntity = pPOPSelect_INSPECT_MIXEntity;
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                _pPOPProductionOrderCommonEntity.CORP_CODE = _pUserEntity.CORP_CODE;
                _pPOPProductionOrderCommonEntity.USER_CODE = _pUserEntity.USER_CODE;
                _pPOPProductionOrderCommonEntity.LANGUAGE_TYPE = _pUserEntity.LANGUAGE_TYPE;
                _pPOPProductionOrderCommonEntity.PROCESS_CODE = _pUserEntity.PROCESS_CODE;
                _pPOPProductionOrderCommonEntity.RESOURCE_CODE = _pUserEntity.RESOURCE_CODE;
        ...
        #region ○ 셀 값 변경에 따른 투입량 계산
        private void gdMAIN_VIEW_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            GridView view = sender as GridView;

            string column = view.FocusedColumn.ToString();

            switch (column)
            {
                //case "배합비율(%)":
                //    view.GetRowCellValue(view., "UPDATE_QTY").ToString();
                //    break;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _pPOPProductionOrderCommonEntity.PRODUCTION_ORDER_ID = _Workorder_ID;

                _dtList = new POPProductionOrderCommonBusiness().POPProductionOrderMix_Info(_pPOPProductionOrderCommonEntity);

                if (_pPOPProductionOrderCommonEntity.CRUD == "") _dtList.Rows.Clear();


                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pPOPProductionOrderCommonEntity.CRUD == ""))
                {
                    ...
                    _gdMAIN_VIEW.CellValueChanged += gdMAIN_VIEW_CellValueChanged;
        ...
        private void btnCmd_Click(object pSender, EventArgs pEventArgs)
        {
            ...
            switch (sCls)
            {
                ...
                case "30":
                    ...
                    _dtreturn.Columns.Add("CRUD", typeof(string));
                    ...
                    _dtreturn.Columns.Add("MATERIAL_CODE", typeof(string));
                    _dtreturn.Columns.Add("MATERIAL_NAME", typeof(string));
                    _dtreturn.Columns.Add("DEFAULT_QTY", typeof(string));
                    _dtreturn.Columns.Add("UPDATE_QTY", typeof(string));
                    _dtreturn.Columns.Add("MATERIAL_UNIT", typeof(string));
                    _dtreturn.Columns.Add("ALLERGY_YN", typeof(string));

                    _dtreturn.Rows.Add(new Object[] {
                        _gdMAIN_VIEW.GetFocusedRowCellValue("CRUD").ToString(),
                        ...
                        _gdMAIN_VIEW.GetFocusedRowCellValue("MATERIAL_CODE").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("MATERIAL_NAME").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("DEFAULT_QTY").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("UPDATE_QTY").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("MATERIAL_UNIT").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("ALLERGY_YN").ToString()
                    });
        ...
        #region ○ 래시피 확인 - Recipe_Check()

        private void Recipe_Check()
        {
            try
            {
                //CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
                ...
                //배합비율 계산
                for (int i = 0; i < _gdMAIN_VIEW.DataRowCount; i++)
                {
                    string check = _gdMAIN_VIEW.GetRowCellValue(i, "UPDATE_QTY").ToString();
                    Boolean check2 = string.IsNullOrEmpty(check);
                    ...
                    num1 = num1 + Convert.ToInt32(Convert.ToDouble(check));

                }

                if (num1 > 100)
                {
                    CoFAS_DevExpressManager.ShowInformationMessage("배합비가 초과 되었습니다.");
                    labelControl14.ForeColor = Color.Red;
                }
                else
                {
                    labelControl14.ForeColor = Color.Black;
                }

                labelControl14.Text = num1.ToString() + " %";


                //투입량 계산
                for (int i = 0; i < _gdMAIN_VIEW.DataRowCount; i++)
                {
                    string STANDARD_QTY = _gdMAIN_VIEW.GetRowCellValue(i, "STANDARD_QTY").ToString();

                    if (string.IsNullOrEmpty(STANDARD_QTY))
                    {
                        STANDARD_QTY = "0";
                    }

                    string BOX_QTY = _gdMAIN_VIEW.GetRowCellValue(i, "BOX_QTY").ToString();

                    if (string.IsNullOrEmpty(BOX_QTY))
                    {
                        BOX_QTY = "0";
                    }

                    string METERAGE_QTY = _gdMAIN_VIEW.GetRowCellValue(i, "METERAGE_QTY").ToString();

                    if (string.IsNullOrEmpty(METERAGE_QTY))
                    {
                        METERAGE_QTY = "0";
                    }

                    int total = Convert.ToInt32(Convert.ToDouble(STANDARD_QTY)) * Convert.ToInt32(Convert.ToDouble(BOX_QTY)) + Convert.ToInt32(Convert.ToDouble(METERAGE_QTY));

                    _gdMAIN_VIEW.SetRowCellValue(i, "INPUT_QTY", total);

                    string check = _gdMAIN_VIEW.GetRowCellValue(i, "INPUT_QTY").ToString();
                    Boolean check2 = string.IsNullOrEmpty(check);
                    ...
                    num2 = num2 + Convert.ToInt32(Convert.ToDouble(check));

                }

                if (num2 > 100)
                {
                    CoFAS_DevExpressManager.ShowInformationMessage("투입량이 초과 되었습니다.");
                ...
                labelControl2.Text = num2.ToString() + " Kg";
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager.TargetSite.ToString()));
