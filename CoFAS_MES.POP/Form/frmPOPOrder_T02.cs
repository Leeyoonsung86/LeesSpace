...
{
    public partial class frmPOPOrder_T02 : frmBaseSingle
    {
        ...
        private int _pFONT_SIZE = 17;                     // 시스템 폰트 사이즈 설정 기본 9
        ...
        public String _pTerminal_code = null;
        ...
        public frmPOPOrder_T02(UserEntity pUserEntity, string terminal_code)
        {
            InitializeComponent();
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                _pPOPProductionOrderCommonEntity.TERMINAL_CODE = _pTerminal_code;
                //CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);

                _dtList = new POPProductionOrderCommonBusiness2().POPProductionOrderCommon_Info2(_pPOPProductionOrderCommonEntity);
                ...
                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pPOPProductionOrderCommonEntity.CRUD == ""))
                {
                    ...
                    _gdMAIN_VIEW.Appearance.Row.Font = new Font("Meiryo UI", 15);
                    _gdMAIN_VIEW.RowStyle += new RowStyleEventHandler(gdMAIN_VIEW_RowStyle);

                    _gdMAIN_VIEW.Columns[4].Visible = false;
                    _gdMAIN_VIEW.Columns[0].Width = 180;
                    _gdMAIN_VIEW.Columns[1].Width = 130;
                    _gdMAIN_VIEW.Columns[2].Width = 100;
                    _gdMAIN_VIEW.Columns[3].Width = 390;
                    //_gdMAIN_VIEW.Columns[4].Width = 70;
                    //_gdMAIN_VIEW.Columns[4].Width = 60;
                    _gdMAIN_VIEW.Columns[5].Width = 60;
                    _gdMAIN_VIEW.Columns[6].Width = 60;
                    _gdMAIN_VIEW.Columns[8].Width = 60;
                    _gdMAIN_VIEW.Columns[7].Width = 50;
                ...
                {
                    CoFAS_DevExpressManager.ShowInformationMessage("照会内容がありません。");
                }
            }
            catch (ExceptionManager pExceptionManager)
            {
        ...
        private void btnCmd_Click(object pSender, EventArgs pEventArgs)
        {
            ...
            switch (sCls)
            {
                ...
                case "30":
                    ...
                    _dtreturn.Columns.Add("작업지시번호", typeof(string));
                    _dtreturn.Columns.Add("지시일자", typeof(string));
                    _dtreturn.Columns.Add("품목코드", typeof(string));
                    _dtreturn.Columns.Add("품목명", typeof(string));
                    _dtreturn.Columns.Add("공정명", typeof(string));
                    _dtreturn.Columns.Add("목표", typeof(string));
                    _dtreturn.Columns.Add("양품", typeof(string));
                    _dtreturn.Columns.Add("불량", typeof(string));
                    _dtreturn.Columns.Add("단위", typeof(string));

                    _dtreturn.Rows.Add(new Object[] {
                        _gdMAIN_VIEW.GetFocusedRowCellValue ("作業指示番号").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("指示日付").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("製品コード").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("製品名").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("作業場").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("目標数量").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("良品数量").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("不良数量").ToString(),
                        _gdMAIN_VIEW.GetFocusedRowCellValue("単位").ToString()                     
