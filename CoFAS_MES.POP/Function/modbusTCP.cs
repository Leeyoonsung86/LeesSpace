...
namespace ModbusTCP
{
    ...
    public class Master
    {
        ...
        public byte[] Local = new byte[2];
        public string AddressReti = string.Empty;
        ...
        private byte[] CreateReadHeader(ushort id, byte unit, ushort startAddress, ushort length, byte function)
        {
            ...
            data[7] = 3;//function;				// Function code
        ...
        private byte[] WriteSyncData(byte[] write_data, ushort id)  // 무선 모듈 변경
        {
            ...
            if (tcpSynCl.Connected)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        Array.Copy(tcpSynClBuffer, 9, data, 0, tcpSynClBuffer[8]);  // 저장할데이터, 시작위치, 저장할 곳, 저장할 위치 , 길이)

                        /////////////////////////////////


                        //data = new byte[tcpSynClBuffer[10]];
                        //Array.Copy(tcpSynClBuffer, 11, data, 0, tcpSynClBuffer[10]);  // 저장할데이터, 시작위치, 저장할 곳, 저장할 위치 , 길이)
                        string LocalSum = tcpSynClBuffer[0].ToString() + tcpSynClBuffer[1].ToString();  //2 /3
                        AddressReti = Convert.ToInt32(LocalSum, 16).ToString();
                    }
                    return data;
                }
                catch (SystemException)
        ...
        // ------------------------------------------------------------------------
        // Byte to Data
        public object ByteToData(byte[] Data)
        {
            ushort num;

            float[] numArray1 = new float[Data.Length / 4];
            for (num = 0; num < Data.Length; num = (ushort)(num + 4))
            {
                byte[] buffer = new byte[] { Data[num + 3], Data[num + 2], Data[num + 1], Data[num] };
                numArray1[num / 4] = BitConverter.ToSingle(buffer, 0);
            }
            return numArray1;
        }
