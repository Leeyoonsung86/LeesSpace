...
{
    public partial class ucTABEquipment : UserControl
    {
        ...
        private ucTABEquipmentEntity _pucTABEquipmentEntity = null; // 엔티티 생성
        ...
        public ucTABEquipment(UserEntity pUserEntity, frmTABMain main)
        {
            InitializeComponent();
        ...
        private void Form_FormClosed(object pSender, FormClosedEventArgs pFormClosedEventArgs)
        {
            try
            {
                ...
                _luCHECK_FILE.ValueChanged -= _luCHECK_FILEValueChanged;
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager.TargetSite.ToString()));
        ...
        private void InitializeControl()
        {
            try
            {
                //그리드 초기화 
                //여러 그리드 사용시 마스터 그리드는 최초 실행 시에만 초기화
                _lciDATE.Text = "DATE " + DateTime.Today.ToString("yyyy/MM/dd");  //점검 시작일, 당일 설정

                _pMonth = DateTime.Today.ToString("yyyyMM");
                _pDay = DateTime.Today.ToString("dd");

                _luCHECK_FILE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "TAB_EQUIPMENT", _pMonth, "", "").Tables[0], 0, 0, "", false);
                _luCHECK_FILE.Font = new Font("Meiryo UI", 16);
                _luCHECK_FILE.Font = new Font("Meiryo UI", 16);

                _luCHECK_FILE.ItemIndex = 0;
        ...
        #region ○ 양식 콤보박스 클릭시 처리하기
        private void _luCHECK_FILEValueChanged(object pSender, EventArgs pEventArgs)
        {
            try
            {
                _pucTABEquipmentEntity.CRUD = "R";
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new ucTABEquipmentBusiness().ucTABEquipment_Info_Return(_pucTABEquipmentEntity);

                if (_pucTABEquipmentEntity.CRUD == "") _dtList.Rows.Clear();

                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pucTABEquipmentEntity.CRUD == ""))
                {
        ...
        private void MainFind_CommentCheck()
        {
            try
            {
                ...
                _dtList_Comment = new ucTABEquipmentBusiness().ucTABEquipment_Comment_Info_Return(_pucTABEquipmentEntity);
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager.TargetSite.ToString()));
        ...
        #region ○ 점검 결과 저장 - InputData_Inspect_Save(DataTable dtSave)

        private void InputData_Inspect_Save()
        {
            try
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
                ...
                isError = new ucTABEquipmentBusiness().ucTABEquipment_Info_Inspect_Save(_pucTABEquipmentEntity);
                ...
                else
                {
                    ...
                    // 단일 리소스 데이터 정보 등록
                    bool isError_Sub = false;

                    isError_Sub = new ucTABRegisterBusiness().ucTABRegister_Info_One_Save(_pucTABRegisterEntity);
