...
{
    public partial class frmPOPLogin_T02 : Form
    {
        ...
        private void frmPOPLogin_T02_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void InitializeControl()
        {
            try
            {
                _luT_PROCESS_LIST.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", pUserEntity.LANGUAGE_TYPE, "POP_LIST_COMMON", "", "", "").Tables[0], 0, 0, "");
        ...
        private void _btLoginOk_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                switch (_SELECT_PROCESS_POP)
                {
                    ...
                    //    pMain1.Show();
                    //    break;
                    case "frmPOPMain_PRODUCT_MIX":    //반죽
                        frmPOPMain_PRODUCT_MIX pMain3 = new frmPOPMain_PRODUCT_MIX(pUserEntity, terminal_code);
                        pMain3.Show();
                        break;
                    case "frmPOPMain_PRODUCT_PACKET":    //포장
                        frmPOPMain_PRODUCT_PACK pMain2 = new frmPOPMain_PRODUCT_PACK(pUserEntity, terminal_code);
                        pMain2.Show();
                        break;
                }
        ...
        {
            switch (_luT_PROCESS_LIST.GetValue())
            {
                case "frmPOPMain_PRODUCT_MIX": //해당 그리드에 필드명 // 2개 이상 그리드의 버튼 항목들은 버튼 타입에 필드 명칭이 모두 다르게 설정 되어야됨.

                    _luT_TERMINAL_LIST.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", pUserEntity.LANGUAGE_TYPE, "POP_TERMINALE_DETAIL", "TP010001", "", "").Tables[0], 0, 0, "");
                    _luT_TERMINAL_LIST.ItemIndex = Properties.Settings.Default.PROCESS_POP;
                    _luT_TERMINAL_LIST.ReadOnly = false;

                    break;

                case "frmPOPMain_PRODUCT_PACKET": //해당 그리드에 필드명 // 2개 이상 그리드의 버튼 항목들은 버튼 타입에 필드 명칭이 모두 다르게 설정 되어야됨.

                    _luT_TERMINAL_LIST.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", pUserEntity.LANGUAGE_TYPE, "POP_TERMINALE_DETAIL", "TP010002", "", "").Tables[0], 0, 0, "");
                    _luT_TERMINAL_LIST.ItemIndex = Properties.Settings.Default.PROCESS_POP;
                    _luT_TERMINAL_LIST.ReadOnly = false;
