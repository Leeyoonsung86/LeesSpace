...
{
    public partial class frmPOPLogin_T08 : Form
    { 
        ...
        private void frmPOPLogin_T08_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void InitializeControl()
        {
            try
            {
                _luT_PROCESS_LIST.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", pUserEntity.LANGUAGE_TYPE, "POP_PROCESS_LIST2", "PT13", "", "").Tables[0], 0, 0, "");
                _luT_PROCESS_LIST.ItemIndex = Properties.Settings.Default.PROCESS_POP;
                _luT_PROCESS_LIST.ReadOnly = false;
                String A = _luT_TERMINAL_LIST.Text;
                string searchText = "(";
                _txtAccount.Text = A.Substring(0, A.IndexOf(searchText));
        ...
        private void InitializeLanguage()
        {
            ...
            _txtAccount.Text = Resources.Language._lciAccount;
            _txtPassword.Text = Resources.Language._lciPassword;
        ...
        private void InitializeCorporationCI()
        {
            ...
            {
                if (System.IO.File.Exists(Application.StartupPath + "\\" + "범아기전_CI.png"))
                {
                    _ucIMG_LOGIN_LOGO.Image.Dispose();
                    _ucIMG_LOGIN_LOGO.Image = null;
                    System.IO.File.Delete(Application.StartupPath + "\\" + "범아기전_CI.png");
                }

                Image imgCI = CoFAS_ConvertManager.byteArrayToImage((byte[])dtCI.Rows[0]["LOGO_FIRST"]);
                if (imgCI != null)
                {
                    imgCI.Save(Application.StartupPath + "\\" + "범아기전_CI.png");
                }
            }

            if (System.IO.File.Exists(Application.StartupPath + "\\" + "범아기전_CI.png"))
            {
                _ucIMG_LOGIN_LOGO.Image = Image.FromFile(Application.StartupPath + "\\" + "범아기전_CI.png");
            }
            else
            {
            ...
            else
            {
                ...
                switch (_SELECT_PROCESS_POP)
                {
                    ...
                    case "PT130001":    //반죽
                        ...
                        break;
                    case "PT130003":    //포장
                        frmPOPMain_PRODUCT pMain2 = new frmPOPMain_PRODUCT(pUserEntity, terminal_code, _luT_TERMINAL_LIST.Text);
                        pMain2.Show();
        ...
        private void _luT_PROCESS_LIST_TabIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (ProcessCode)
            {
                case "PT130003": //해당 그리드에 필드명 // 2개 이상 그리드의 버튼 항목들은 버튼 타입에 필드 명칭이 모두 다르게 설정 되어야됨.
                    ...
                    _luT_TERMINAL_LIST.ItemIndex = Properties.Settings.Default.PROCESS_POP;
                    _luT_TERMINAL_LIST.ReadOnly = false;
                    _txtAccount.Text = _luT_TERMINAL_LIST.Text;
                    break;
        ...
        #region ○ 단말기 구분 콤보박스 변경 - _luT_PROCESS_LIST_TabIndexChanged(object sender, EventArgs e)
        private void _luT_Terminal_LIST_TabIndexChanged(object sender, EventArgs e)
        {
            String TerminalCode = _luT_TERMINAL_LIST.GetValue();
            String A = _luT_TERMINAL_LIST.Text;
            string searchText = "(";
            _txtAccount.Text = A.Substring(0, A.IndexOf(searchText));
        ...
        private void btnCmd_Click(object pSender, EventArgs pEventArgs)
        {
            ...
            switch (sCls)
            {
                case "00":
                    ...
                    {
                        if (_txtPassword.Text == "-00" || _txtPassword.Text == "00")
                        {
                            _txtPassword.Text = _txtPassword.Text.Substring(0, _txtPassword.Text.Length - 1);
                        }
                    }
                ...
                //        if (decimal.TryParse(_txtPassword.Text, out temp))
                //        {
                //            _txtPassword.Text = decimal.Parse(_txtPassword.Text).ToString();


                //            this.DialogResult = DialogResult.OK;

                //            ReturnValue1 = _txtPassword.Text;
                //            this.Close();
                //        }
                //        else
                //        {
                //            if (_txtPassword.Text == "0" || _txtPassword.Text.Trim() == "")
                //            {
                //                return;
                //            }
                //        }
                ...
                case "60":
                    //- 마이너스
                    if (_txtPassword.Text.IndexOf('-') < 0)
                    {
                        _txtPassword.Text = _txtPassword.Text.Insert(0, "-");
                    }
                    break;

                case "70":
                    //+ 플러스
                    if (_txtPassword.Text.IndexOf('-') > -1)
                    {
                        _txtPassword.Text = _txtPassword.Text.Replace("-", string.Empty);
                    }
                    break;
