...
{
    public partial class EquipmentCheckListMonth_T50 : frmBaseNone
    {
        ...
        private EquipmentCheckListMonth_T50Entity _pEquipmentCheckListMonth_T50Entity = null; // 엔티티 생성
        ...
        public EquipmentCheckListMonth_T50()
        {
            InitializeComponent();
        ...
        private void InitializeSetting()
        {
            try
            {
                _pEquipmentCheckListMonth_T50Entity = new EquipmentCheckListMonth_T50Entity();
                //상속 화면 패널 사용 유무
        ...
        private void InitializeControl()
        {
            try
            {
                ...
                _luProcessList.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "POP_PROCESS_DSELECT", "", "", "").Tables[0], 0, 0, "");
                _luProcessList.ItemIndex = 0;
                _luProcessList.ReadOnly = false;
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager.TargetSite.ToString()));
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                _pEquipmentCheckListMonth_T50Entity.CRUD = "R";
        ...
        {
            _pEquipmentCheckListMonth_T50Entity.WINDOW_NAME = this.Name;

            string strUSE_TYPE = "";
            string strFILE_NAME = "";
            string strFTP_PATH = "";

            _dtList = new EquipmentCheckListMonth_T50Business().Sheet_Info_Sheet(_pEquipmentCheckListMonth_T50Entity, _luProcessList.Text);
        ...
        public void SetData()
        {
            ...
            worksheet.Cells["N1"].Value = _luDATE.DateTime.Year + "년" + _luDATE.DateTime.Month + "월";

            _dtList = new EquipmentCheckListMonth_T50Business().USP_equipment_check_result_R10(_luDATE.DateTime.ToShortDateString().Substring(0, 7), _luProcessList.Text, 1);

            if(_dtList != null && _dtList.Rows.Count > 0)
            {
                for(int i = 0; i < _dtList.Rows.Count; i++)
                {
                    if (worksheet.Cells[i + 4, 1].Value.ToString() == "")
                    {
                        worksheet.Cells[i + 4, 1].Value = _dtList.Rows[i]["항목1"].ToString();
                    }
                    if (worksheet.Cells[i + 4, 2].Value.ToString() == "")
                    {
                        worksheet.Cells[i + 4, 2].Value = _dtList.Rows[i]["항목2"].ToString();
                    }
                    if (worksheet.Cells[i + 4, 3].Value.ToString() == "")
                    {
                        worksheet.Cells[i + 4, 3].Value = _dtList.Rows[i]["점검기준"].ToString();
                    }
                    if (worksheet.Cells[i + 4, 4].Value.ToString() == "")
                    {
                        worksheet.Cells[i + 4, 4].Value = _dtList.Rows[i]["점검주기"].ToString();
                    }                    
                }
            }

            _dtList = new EquipmentCheckListMonth_T50Business().USP_equipment_check_result_R10(_luDATE.DateTime.ToShortDateString().Substring(0, 7), _luProcessList.Text, 2);
            ...
            if (_dtList != null && _dtList.Rows.Count > 0)
            {
                for (int i = 0; i < _dtList.Rows.Count; i++)
                {
                    ...
                    for (j = 5; j < 36; j++)
                    {
                        if (worksheet.Cells[3, j].Value == Convert.ToDateTime(_dtList.Rows[i]["date"].ToString()).Day)
                        {
                            break;
                        }
                    }

                    worksheet.Cells[i + 4, j].Value = _dtList.Rows[i]["result"].ToString();
                }
            }
        }
    }
