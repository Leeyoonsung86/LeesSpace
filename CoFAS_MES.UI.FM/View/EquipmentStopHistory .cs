...
{
    public partial class EquipmentStopHistory : frmBaseNone
    {
        ...
        private EquipmentStopHistoryEntity _pEquipmentStopHistoryEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pEquipmentStopHistoryEntity = new EquipmentStopHistoryEntity();
                //_pProductMonitoring_T01Entity.CORP_CODE = _pCORP_CODE;
                _pEquipmentStopHistoryEntity.USER_CODE = _pUSER_CODE;
                _pEquipmentStopHistoryEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pEquipmentStopHistoryEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pEquipmentStopHistoryEntity.CRUD = "";
                if (_pFirstYN)
                {
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                string _check_list = _luTSTOP_MST.Text;

                _pEquipmentStopHistoryEntity.CRUD = "R";
                _pEquipmentStopHistoryEntity.DATE_FROM = DateTime.Parse(_luTORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");
                _pEquipmentStopHistoryEntity.DATE_TO = DateTime.Parse(_luTORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");
                _pEquipmentStopHistoryEntity.EQUIPMENT_CODE = _luTTERMINAL_LIST.GetValue();
                _pEquipmentStopHistoryEntity.USER_CODE = _pUSER_CODE;
                _pEquipmentStopHistoryEntity.STOP_MST = _luTSTOP_MST.GetValue();
                _pEquipmentStopHistoryEntity.STOP_DETAIL = _luTSTOP_DETAIL.GetValue();




                _dtList = new EquipmentStopHistoryBusiness().EquipmentStopHistory_Mst(_pEquipmentStopHistoryEntity); //Entity에 WINDOW_NAME 추가

                if (_pEquipmentStopHistoryEntity.CRUD == "")
                    _dtList.Rows.Clear();

                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pEquipmentStopHistoryEntity.CRUD == ""))
                {
        ...
        private void _luTSTOP_MST_ValueChanged(object sender, EventArgs e)
        {
            _luTSTOP_DETAIL.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", "", "STOP_DETAIL_YB", _luTSTOP_MST.GetValue(), "", "").Tables[0], 0, 0, "",true);
        }
    }
