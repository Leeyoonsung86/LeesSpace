...
{
    public partial class InspectCodeMstRegister : frmBaseNone
    {
        ...
        private InspectCodeMstRegisterEntity _pInspectCodeMstRegisterEntity = null; // 엔티티 생성
        ...
        private void Form_FormClosed(object pSender, FormClosedEventArgs pFormClosedEventArgs)
        {
            try
            {
                ...
                _luT_INSPECT._OnOpenClick -= _luT_INSPECT_OnOpenClick;
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pInspectCodeMstRegisterEntity = new InspectCodeMstRegisterEntity();
                //_pInspectCodeMstRegisterEntity.CORP_CODE = _pCORP_CODE;
                _pInspectCodeMstRegisterEntity.USER_CODE = _pUSER_CODE;
                _pInspectCodeMstRegisterEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pInspectCodeMstRegisterEntity.CRUD = "";
        ...
        private void InitializeControl()
        {
            try
            {
               ...
               // _luPROCESS_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "PROCESS_TYPE", "", "", "").Tables[0], 0, 0, "");
                ...
                _luINSPECT_GROUP_CODE.Text = "";
                _luINSPECT_GROUP_NAME.Text = "";
                _luT_INSPECT.CodeText = "";
                _luT_INSPECT.NameText = "";
                _luREMARK.Text = "";
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                string strPROCESS_CODE = gv.GetFocusedRowCellValue("INSPECT_GROUP_CODE").ToString();

                _pInspectCodeMstRegisterEntity.CRUD = "R";
                _pInspectCodeMstRegisterEntity.INSPECT_GROUP_CODE = gv.GetFocusedRowCellValue("INSPECT_GROUP_CODE").ToString();
        ...
        private void _gdSUB_VIEW_InitNewRow(object sender, InitNewRowEventArgs e)
        {
            ...
            if (_luINSPECT_GROUP_CODE.Text != "")
                view.SetRowCellValue(e.RowHandle, view.Columns["INSPECT_GROUP_CODE"], _luINSPECT_GROUP_CODE.Text);
        }
        #endregion
        ...
        #region ○ 조회 서치박스 - _luT_INSPECT_OnOpenClick(object sender, EventArgs e)
        private void _luT_INSPECT_OnOpenClick(object sender, EventArgs e)
        {
            try
            {
                frmCommonPopup.CRUD = "R";
                ...
                frmCommonPopup.ARRAY = new object[2] { "Inspect_Code", "" };
                frmCommonPopup.ARRAY_CODE = new object[2] { _luT_INSPECT.CodeText.ToString(), _luT_INSPECT.NameText.ToString() };
                frmCommonPopup xfrmCommonPopup = new CORE.UserForm.frmCommonPopup("CommonCode"); //유저컨트롤러 설정 부분
                ...
                if (xfrmCommonPopup.dtReturn != null && xfrmCommonPopup.dtReturn.Rows.Count > 0)
                {
                    _luT_INSPECT.CodeText = xfrmCommonPopup.dtReturn.Rows[0]["CD"].ToString();
                    _luT_INSPECT.NameText = xfrmCommonPopup.dtReturn.Rows[0]["CD_NM"].ToString();
                }
        ...
        private void Form_SaveButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                if (_luINSPECT_GROUP_NAME.Text.Length < 1)
                {
                    return;
                }

                //확인
                if (_luINSPECT_GROUP_CODE.Text.Length > 0)
                {
                    _pInspectCodeMstRegisterEntity.CRUD = "U";
                }
                else
                {
                    _pInspectCodeMstRegisterEntity.CRUD = "C";
                }

                _pInspectCodeMstRegisterEntity.INSPECT_GROUP_CODE = _luINSPECT_GROUP_CODE.Text.ToString();
                _pInspectCodeMstRegisterEntity.INSPECT_GROUP_NAME = _luINSPECT_GROUP_NAME.Text.ToString();
                _pInspectCodeMstRegisterEntity.REMARK = _luREMARK.Text.ToString();
                _pInspectCodeMstRegisterEntity.USE_YN = _luUSE_YN.GetValue();
        ...
        private void MainFind_DisplayData()
        {
            ...
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
                _pInspectCodeMstRegisterEntity.INSPECT_GROUP_CODE = _luT_INSPECT.CodeText.ToString();
                _pInspectCodeMstRegisterEntity.INSPECT_GROUP_NAME = _luT_INSPECT.NameText.ToString();
                _pInspectCodeMstRegisterEntity.USE_YN = _luT_USE_YN.GetValue();


                _dtList = new InspectCodeMstRegisterBusiness().Inspect_Info_Mst(_pInspectCodeMstRegisterEntity);

                if (_pInspectCodeMstRegisterEntity.CRUD == "") _dtList.Rows.Clear();


                if (_dtList != null && _dtList.Rows.Count > 0)//&& _dtList.Rows.Count > 0) )|| (_dtList != null && _pInspectCodeMstRegisterEntity.CRUD == ""))
                {
                    CoFAS_DevExpressManager.BindGridControl(_gdMAIN, _gdMAIN_VIEW, _dtList); //데이터 필드에 맞춰 자동 바인딩

                    if (_pLocation_Code != "" && _pLocation_YN)
                    {
                        int rowHandle = _gdMAIN_VIEW.LocateByValue("INSPECT_GROUP_CODE", _pLocation_Code);
        ...
        #region ○ 그리드 셀 속성
        /*
        private void _gdSUB_VIEW_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            GridView view = sender as GridView;
            if (e.Column != view.Columns["INSPECT_VALUE"])
                return;

            e.Appearance.BackColor = Color.LightSkyBlue;
            e.Appearance.Font = new Font("굴림", 10, FontStyle.Bold);
            //e.Column.ColumnType=gridviewcolumn
        }
        */
        #endregion
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
                ...
                isError = new InspectCodeMstRegisterBusiness().Inspect_Info_Save(_pInspectCodeMstRegisterEntity, dtSave);
                ...
                else
                {
                    ...
                    // 화면 갱신
                    _pInspectCodeMstRegisterEntity.CRUD = "R";
                    _pInspectCodeMstRegisterEntity.PROCESS_CODE = "";
                    //_pInspectCodeMstRegisterEntity.PROCESS_NAME = "";
                    _pInspectCodeMstRegisterEntity.USE_YN = _luT_USE_YN.GetValue();

                    _pLocation_Code = _pInspectCodeMstRegisterEntity.RTN_KEY;  //저장 위치 
