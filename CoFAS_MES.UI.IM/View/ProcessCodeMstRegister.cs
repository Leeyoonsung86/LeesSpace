...
{
    public partial class ProcessCodeMstRegister : frmBaseNone
    {
        ...
        private ProcessCodeMstRegisterEntity _pProcessCodeMstRegisterEntity = null; // 엔티티 생성
        ...
        private void Form_FormClosed(object pSender, FormClosedEventArgs pFormClosedEventArgs)
        {
            try
            {
                ...
                _luT_PROCESS_MST._OnOpenClick -= _luT_PROCESS_OnOpenClick;
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pProcessCodeMstRegisterEntity = new ProcessCodeMstRegisterEntity();
                //_pProcessCodeMstRegisterEntity.CORP_CODE = _pCORP_CODE;
                _pProcessCodeMstRegisterEntity.USER_CODE = _pUSER_CODE;
                _pProcessCodeMstRegisterEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                if (_pFirstYN)
                {
                    ...
                    //_luT_PROCESS_CODE.Text = "";
                    //_luT_PROCESS_NAME.Text = "";
        ...
        private void InitializeControl()
        {
            try
            {
                ...
                _luPROCESS_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R",_pLANGUAGE_TYPE, "PROCESS_TYPE", "", "", "").Tables[0], 0, 0, "");

                _luPROCESS_MST_CODE.Text = "";
                _luPROCESS_MST_NAME.Text = "";
                _luT_PROCESS_MST.CodeText = "";
                _luT_PROCESS_MST.NameText = "";
                _luREMARK.Text = "";
                _luT_USE_YN.ItemIndex = 1;
                //_ucbtPROCESS_MST_DOCUMENT_BTN.Visible = true;
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                string strPROCESS_CODE = gv.GetFocusedRowCellValue("PROCESS_MST_CODE").ToString();

                _pProcessCodeMstRegisterEntity.CRUD = "R";
                _pProcessCodeMstRegisterEntity.PROCESS_MST_CODE = gv.GetFocusedRowCellValue("PROCESS_MST_CODE").ToString();
        ...
        private void CoFAS_DevExpressManager__OnButtonPressed(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            {
                case "PROCESS_DOCUMENT_BTN": //해당 그리드에 필드명 // 2개 이상 그리드의 버튼 항목들은 버튼 타입에 필드 명칭이 모두 다르게 설정 되어야됨.
                    if (dr["PROCESS_CODE"].ToString() == "") return;
                    ...
                    UserForm.frmCommonPopup.ARRAY_CODE = new object[2] { dr["PROCESS_CODE"].ToString(), dr["PROCESS_NAME"].ToString() };// dr["CONTRACT_ID"].ToString(), dr["CONTRACT_NUMBER"].ToString() }; //넘기는 파라메터 에 따라 설정

                    UserForm.frmCommonPopup xfrmcommonPopup = new UserForm.frmCommonPopup("ucProcessDocumentListPopup"); //유저컨트롤러 설정 부분
        ...
        #region ○ 조회 서치박스 - _luT_PROCESS_OnOpenClick(object sender, EventArgs e)
        private void _luT_PROCESS_OnOpenClick(object sender, EventArgs e)
        {
            try
            {
                ...
                frmCommonPopup.ARRAY_CODE = new object[2] { _luT_PROCESS_MST.CodeText.ToString(), _luT_PROCESS_MST.NameText.ToString() };
                //20190321 기존 공통팝업용
                ...
                frmCommonPopup xfrmCommonPopup = new CORE.UserForm.frmCommonPopup("CommonCode_ProcessGroup"); //유저컨트롤러 설정 부분
                ...
                {
                    _luT_PROCESS_MST.CodeText = xfrmCommonPopup.dtReturn.Rows[0]["CD"].ToString();
                    _luT_PROCESS_MST.NameText = xfrmCommonPopup.dtReturn.Rows[0]["CD_NM"].ToString();
                }
        ...
        private void Form_SaveButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                if(_luPROCESS_MST_NAME.Text.Length < 1)
                {
                    return;
                }

                //확인
                if (_luPROCESS_MST_CODE.Text.Length > 0)
                {
                    _pProcessCodeMstRegisterEntity.CRUD = "U";
                }
                else
                {
                    _pProcessCodeMstRegisterEntity.CRUD = "C";
                }
                _pProcessCodeMstRegisterEntity.RTN_KEY = "";
                _pProcessCodeMstRegisterEntity.PROCESS_MST_CODE = _luPROCESS_MST_CODE.Text.ToString();
                _pProcessCodeMstRegisterEntity.PROCESS_MST_NAME = _luPROCESS_MST_NAME.Text.ToString();
                _pProcessCodeMstRegisterEntity.PROCESS_TYPE = _luPROCESS_TYPE.GetValue();
                _pProcessCodeMstRegisterEntity.REMARK = _luREMARK.Text.ToString();
                _pProcessCodeMstRegisterEntity.USE_YN = _luUSE_YN.GetValue();
        ...
        private void MainFind_DisplayData()
        {
            ...
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
                _pProcessCodeMstRegisterEntity.PROCESS_MST_CODE = _luT_PROCESS_MST.CodeText.ToString();
                _pProcessCodeMstRegisterEntity.PROCESS_MST_NAME = _luT_PROCESS_MST.NameText.ToString();
                _pProcessCodeMstRegisterEntity.PROCESS_TYPE = _luT_PROCESS_TYPE.GetValue();
                _pProcessCodeMstRegisterEntity.USE_YN = _luT_USE_YN.GetValue();


                _dtList = new ProcessCodeMstRegisterBusiness().Process_Info_Mst(_pProcessCodeMstRegisterEntity);

                if (_pProcessCodeMstRegisterEntity.CRUD == "") _dtList.Rows.Clear();


                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pProcessCodeMstRegisterEntity.CRUD == ""))
                {
                    CoFAS_DevExpressManager.BindGridControl(_gdMAIN, _gdMAIN_VIEW, _dtList); //데이터 필드에 맞춰 자동 바인딩

                    if(_pLocation_Code != "" && _pLocation_YN)
                    {
                        int rowHandle = _gdMAIN_VIEW.LocateByValue("PROCESS_MST_CODE", _pLocation_Code);
        ...
        private void MainBindingFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new ProcessCodeMstRegisterBusiness().Process_Info_Mst_Binding(_pProcessCodeMstRegisterEntity);
        ...
        private void SubFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new ProcessCodeMstRegisterBusiness().Process_Info_Detail(_pProcessCodeMstRegisterEntity);
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
                ...
                isError = new ProcessCodeMstRegisterBusiness().Process_Info_Save(_pProcessCodeMstRegisterEntity, dtSave);
                ...
                else
                {
                    ...
                    // 화면 갱신
                    _pProcessCodeMstRegisterEntity.CRUD = "R";
                    _pProcessCodeMstRegisterEntity.PROCESS_MST_CODE = "";
                    _pProcessCodeMstRegisterEntity.PROCESS_MST_NAME = "";
                    _pProcessCodeMstRegisterEntity.USE_YN = _luT_USE_YN.GetValue();

                    _pLocation_Code = _pProcessCodeMstRegisterEntity.RTN_KEY;  //저장 위치 
        ...
        private void _ucbtPROCESS_MST_DOCUMENT_BTN_Click(object sender, EventArgs e)
        {
            UserForm.frmCommonPopup.CORP_CDDE = _pCORP_CODE;
            UserForm.frmCommonPopup.USER_CODE = _pUSER_CODE;
            UserForm.frmCommonPopup.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
            ...
            UserForm.frmCommonPopup.ARRAY = new object[2] { "", "" }; //넘기는 파라메터 에 따라 설정
            UserForm.frmCommonPopup.ARRAY_CODE = new object[2] { _luPROCESS_MST_CODE.Text.ToString(), _luPROCESS_MST_NAME.Text.ToString() };// dr["CONTRACT_ID"].ToString(), dr["CONTRACT_NUMBER"].ToString() }; //넘기는 파라메터 에 따라 설정
        ...
        private void _luPROCESS_MST_CODE__OnTextChanged(object sender, EventArgs e)
        {
            if(_luPROCESS_MST_CODE.Text.ToString().Length > 0)
            {
                _ucbtPROCESS_MST_DOCUMENT_BTN.Enabled = true;
            }
            else
            {
                _ucbtPROCESS_MST_DOCUMENT_BTN.Enabled = false;
            }
        }
