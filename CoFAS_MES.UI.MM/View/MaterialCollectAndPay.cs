...
{
    public partial class MaterialCollectAndPay : frmBaseNone
    {
        ...
        private MaterialCollectAndPayEntity _pMaterialCollectAndPayEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pMaterialCollectAndPayEntity = new MaterialCollectAndPayEntity();
                //_pMaterialCollectAndPayEntity.CORP_CODE = _pCORP_CODE;
                _pMaterialCollectAndPayEntity.USER_CODE = _pUSER_CODE;
                _pMaterialCollectAndPayEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pMaterialCollectAndPayEntity.CRUD = "";
                if (_pFirstYN)
                {
        ...
        private void _gdMAIN_VIEW_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                // _pMaterialCollectAndPayEntity.CRUD = "R";
                //   SubFind_DisplayData(strWINDOW_NAME, strGRID_NAME);
                /*
                _pMaterialCollectAndPayEntity.DATE_FROM = DateTime.Parse(_luORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");//Convert.ToString(_luORDER_DATE.FromDateTime).Substring(0, 10).Replace("-", ""); //추가 수정
                _pMaterialCollectAndPayEntity.DATE_TO = DateTime.Parse(_luORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd"); // Convert.ToString(_luORDER_DATE.ToDateTime).Substring(0, 10).Replace("-", "");      //추가 수정
                _pMaterialCollectAndPayEntity.VEND_NAME = _luT_VEND_NAME.Text;
                _pMaterialCollectAndPayEntity.PART_NAME = _luPART_NAME.Text;
                _pMaterialCollectAndPayEntity.VEND_PART_CODE = _luT_VEND_PART_CODE.Text;
                _pMaterialCollectAndPayEntity.PART_TYPE = _luT_PART_TYPE.GetValue();
                _pMaterialCollectAndPayEntity.PART_CODE = _luPART_CODE.Text;
                 */
                _pMaterialCollectAndPayEntity.PART_CODE = gv.GetFocusedRowCellValue("PART_CODE").ToString();
               ...
               CoFAS_MES.UI.MM.UserForm.frmCommonPopup.USER_CODE = _pUSER_CODE;
                ...
                CoFAS_MES.UI.MM.UserForm.frmCommonPopup.ARRAY = new object[2] { "common_code", "CD99" }; //넘기는 파라메터 에 따라 설정
                CoFAS_MES.UI.MM.UserForm.frmCommonPopup.ARRAY_CODE = new object[3] { _pMaterialCollectAndPayEntity.DATE_FROM, _pMaterialCollectAndPayEntity.DATE_TO, _pMaterialCollectAndPayEntity.PART_CODE };

                CoFAS_MES.UI.MM.UserForm.frmCommonPopup xfrmcommonPopup = new CoFAS_MES.UI.MM.UserForm.frmCommonPopup("MaterialCollectAndPay_Detail"); //유저컨트롤러 설정 부분
        ...
        private void InitializeControl()
        {
            ...
            {
                //조회조건 영역 
                // PART_TYPE_PRODUCT_T01
                _luT_PART_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "PART_TYPE_MATERIAL_T04", "", "", "").Tables[0], 0, 0, "");
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new MaterialCollectAndPayBusiness().Sample_Info(_pMaterialCollectAndPayEntity);

                if (_pMaterialCollectAndPayEntity.CRUD == "") _dtList.Rows.Clear();
                ...
                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pMaterialCollectAndPayEntity.CRUD == ""))
                {
                    ...
                    //이월재고 관련
                    /*
                    //_gdMAIN_VIEW.Columns["BEFORE_STOCK"].GroupIndex = -1;
                    GridGroupSummaryItem GroupSummary = new GridGroupSummaryItem();
                    GroupSummary.FieldName = "BEFORE_STOCK";
                    GroupSummary.ShowInGroupColumnFooter = _gdMAIN_VIEW.Columns["BEFORE_STOCK"];
                    //GroupSummary.SummaryType = DevExpress.Data.SummaryItemType.Sum;
                   // GroupSummary.DisplayFormat = "Sum = {0:c2}";
                    _gdMAIN_VIEW.GroupSummary.Add(GroupSummary);
                    */
                }
                else
                {
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
              ...
              //  isError = new MaterialCollectAndPayBusiness().Sample_Info_Save(_pMaterialCollectAndPayEntity, dtSave);
