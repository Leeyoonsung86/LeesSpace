...
{
    public partial class MaterialInStatus : frmBaseNone
    {
        ...
        private MaterialInStatusEntity _pMaterialInStatusEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pMaterialInStatusEntity = new MaterialInStatusEntity();
                //_pMaterialInStatusEntity.CORP_CODE = _pCORP_CODE;
                _pMaterialInStatusEntity.USER_CODE = _pUSER_CODE;
                _pMaterialInStatusEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pMaterialInStatusEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                //화면 설정 언어 & 명칭 변경.
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                _pMaterialInStatusEntity.CRUD = "R";
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager));
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                _pMaterialInStatusEntity.CRUD = "R";
                _pMaterialInStatusEntity.DATE_FROM = DateTime.Parse(_luTORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");//Convert.ToString(_luTORDER_DATE.FromDateTime).Substring(0, 10).Replace("-", "");
                _pMaterialInStatusEntity.DATE_TO = DateTime.Parse(_luTORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd"); //Convert.ToString(_luTORDER_DATE.ToDateTime).Substring(0, 10).Replace("-", "");
                _pMaterialInStatusEntity.PART_CODE = _luTINOUT_ID.Text;
                _pMaterialInStatusEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                Dashboard_DisplayData();
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                //_dtList = new MaterialInStatusBusiness().MaterialInStatus_Info_Filename(_pMaterialInStatusEntity);

                //if (_pMaterialInStatusEntity.CRUD == "") _dtList.Rows.Clear();


                //if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pMaterialInStatusEntity.CRUD == ""))
                //{
