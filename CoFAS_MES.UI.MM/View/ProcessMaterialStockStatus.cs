...
{
    public partial class ProcessMaterialStockStatus : frmBaseDesign
    {
        ...
        private ProcessMaterialStockStatusEntity _pProcessMaterialStockStatusEntity = null; // 엔티티 생성

        private DataTable _dtList = null; //조회 데이터 리스트

        private object _obList = null;

        private Object _ojList = null;
        private DataSet _stList = null;

        private bool _pFirstYN = true;                  // 최초실행여부 최초 설정 에서만 사용

        public ProcessMaterialStockStatus()
        {
            InitializeComponent();
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pProcessMaterialStockStatusEntity = new ProcessMaterialStockStatusEntity();
                //_pProcessMaterialStockStatusEntity.CORP_CODE = _pCORP_CODE;
                _pProcessMaterialStockStatusEntity.USER_CODE = _pUSER_CODE;
                _pProcessMaterialStockStatusEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pProcessMaterialStockStatusEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pProcessMaterialStockStatusEntity.CRUD = "";
                if (_pFirstYN)
                {
                ...
                // DashboardViewer  XML 파일 확인 및 띄우기
                _dtList = new ProcessMaterialStockStatusBusiness().Sample_Info_Filename(_pProcessMaterialStockStatusEntity); //Entity에 WINDOW_NAME 추가

                for (int i = 0; i < _dtList.Rows.Count; i++)
                {
                    string menu_no = _dtList.Rows[i][0].ToString();
                    string dsp_sort = _dtList.Rows[i][1].ToString();
                    ...
                    if (!File.Exists(curfile))  // 같은 것이 있으면, 폴더에서 해당 파일명을 불러와서 DashboardViewer.DashboardSource에 띄우기 / 없으면, 폴더에 다운
                    {
                        ...
                        _obList = new ProcessMaterialStockStatusBusiness().Templete_Download(_pProcessMaterialStockStatusEntity, menu_no, dsp_sort);
                    ...
                    else
                    {
                        ...
                        dashboardViewer.DashboardSource = new System.Uri(curfile1, System.UriKind.Absolute);
                    }
                }
        ...
        private void InitializeGrid()
        {
            try
            {
                ...
                if (_pFirstYN)
                {
                    // _gdMAIN_VIEW = CoFAS_DevExpressManager.Grid_Setting(_gdMAIN, _gdMAIN_VIEW, _pCORP_CDDE, _pFONT_SETTING, _pLANGUAGE_TYPE, new DevGridSettingBusiness().DevGrid_Info(_pCORP_CDDE, _pWINDOW_NAME, _gdMAIN.Name.ToString()));
                }
        ...
        private void InitializeControl()
        {
            ...
            {
                //조회조건 영역 
                _luTPROCESS_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R",_pLANGUAGE_TYPE, "processcode", "", "", "").Tables[0], 0, -1, "");
                _luTPART_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R",_pLANGUAGE_TYPE,      "PART_TYPE", "", "", "").Tables[0], 0, -1, "");
                _luTTIME_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R",_pLANGUAGE_TYPE, "common_code", "CD96", "", "").Tables[0], 0, -1, "");
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                _pProcessMaterialStockStatusEntity.CRUD = "R";
        ...
        private void CoFAS_DevExpressManager__OnButtonPressed(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            switch (e.Button.Caption.ToString())
            {
                case "TEST": //해당 그리드에 필드명 // 2개 이상 그리드의 버튼 항목들은 버튼 타입에 필드 명칭이 모두 다르게 설정 되어야됨.
                    //CORE.UserForm.frmCommonPopup.CORP_CDDE = _pCORP_CODE;
                    ...
                    CORE.UserForm.frmCommonPopup.ARRAY = new object[2] { "common_code", "CD99" }; //넘기는 파라메터 에 따라 설정

                    CORE.UserForm.frmCommonPopup xfrmcommonPopup = new CORE.UserForm.frmCommonPopup("CommonCode"); //유저컨트롤러 설정 부분
        ...
        #region ○ Dashboard Parameter to a Custom SQL Query in Code
        private void Dashboard_Parameter()
        {
            try
            {
                ...
                salesPersonQuery.Parameters[0].Value = "ProcessMaterialStockStatus";

                //salesPersonQuery.Parameters.Add(new QueryParameter("window_name", typeof(Expression),
        ...
        private void Form_SettingButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                //{
                //    using (CoFAS_MES.CORE.UserForm.frmViewDesign designerForm = new CoFAS_MES.CORE.UserForm.frmViewDesign(null))
                //    {
                //        //designerForm.PASS_CORP_CODE = _pCORP_CODE;
                //        designerForm.PASS_PARENT_WINDOW_NAME = this.Text;
                //        designerForm.PASS_PARENT_WINDOW_ID = _pWINDOW_NAME;
                //        designerForm.PASS_USER_CODE = _pUSER_CODE;
                //        designerForm.PASS_PARENT_LANGUAGE = _pLANGUAGE_TYPE;
                //        designerForm.PASS_PARENT_FONT = _pFONT_SETTING;
                ...
                //else
                //{
                //    using (Dashboard dashboard = CreateCopy(dashboardViewer.Dashboard))
                //    {
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new ProcessMaterialStockStatusBusiness().Sample_Info(_pProcessMaterialStockStatusEntity);

                if (_pProcessMaterialStockStatusEntity.CRUD == "") _dtList.Rows.Clear();


                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pProcessMaterialStockStatusEntity.CRUD == ""))
                {
        ...
        private void SubFind_DisplayData(string strWINDOW_NAME, string strGRID_NAME)
        {
            try
            {
                ...
                _dtList = new ProcessMaterialStockStatusBusiness().Sample_Info_Sub(_pProcessMaterialStockStatusEntity);
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
                ...
                isError = new ProcessMaterialStockStatusBusiness().Sample_Info_Save(_pProcessMaterialStockStatusEntity, dtSave);
                ...
                else
                {
                    ...
                    // SubFind_DisplayData(_pProcessMaterialStockStatusEntityBusinessEntity.WINDOW_NAME, _pProcessMaterialStockStatusEntityBusinessEntity.GRID_NAME);
                }
