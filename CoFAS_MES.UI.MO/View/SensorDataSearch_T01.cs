...
{
    public partial class SensorDataSearch_T01 : frmBaseNone
    {
        ...
        private SensorDataSearchEntity _pSensorDataSearchEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pSensorDataSearchEntity = new SensorDataSearchEntity();
                //_pSensorDataSearchEntity.CORP_CODE = _pCORP_CODE;
                _pSensorDataSearchEntity.USER_CODE = _pUSER_CODE;
                _pSensorDataSearchEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pSensorDataSearchEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
        ...
        private void InitializeControl()
        {
            try
            {
                ...
                _luT_TERM.ReadOnly = false;
                _luT_TERM.DisableTextEditor();
                ...
                if (_pFirstYN)
                {
                    _luT_TERM.ItemAdd("Day");
                    _luT_TERM.ItemAdd("Hour");
                    _luT_TERM.ItemAdd("Minute");
                    _luT_TERM.SelectedIndex = 0;
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                _pSensorDataSearchEntity.RESOURCE_CODE = sList.Replace("''", "','").Replace("'", "").ToString();
                _pSensorDataSearchEntity.FROM_DATE = _luT_DATE.FromDateTime.ToString().Substring(0, 10);
                _pSensorDataSearchEntity.TO_DATE = _luT_DATE.ToDateTime.ToString().Substring(0, 10);
                _pSensorDataSearchEntity.INTERVAL = _luT_TERM.SelectedItem.ToString();
                ...
                if (chk == 1)
                {
                    if (_luT_TERM.SelectedItem.ToString() == "Minute")
                    {
                        if ((ltdate.Date - stdate.Date).Days > 10)
                        {
                            CoFAS_DevExpressManager.ShowErrorMessage(_pMSG_CHECK_SEARCH_DATE + "\r\n(The 'Minute' interval is maximum for a week)");
                            return;
                        }
                    }

                    if (_luT_TERM.SelectedItem.ToString() == "Hour")
                    {
                        if ((ltdate.Date - stdate.Date).Days > 125)
                        {
                            CoFAS_DevExpressManager.ShowErrorMessage(_pMSG_CHECK_SEARCH_DATE + "\r\n(The 'Hour' interval is maximum for 4 months.)");
                            return;
                        }
                    }
                }
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                if (_pSensorDataSearchEntity.CRUD == "") _dtMList.Rows.Clear();
