...
{
    public partial class SensorStatusSeacrh_T01 : frmBaseNone
    {
        ...
        private SensorStatusSeacrh_T01Entity _pSensorStatusSeacrh_T01Entity = null; // 엔티티 생성
        ...
        private DataTable _dtList = null; // 대쉬보드 정보 조회
        private object _obList = null;

        private bool _pFirstYN = true;                  // 최초실행여부 최초 설정 에서만 사용


        public SensorStatusSeacrh_T01()
        {
            InitializeComponent();
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pSensorStatusSeacrh_T01Entity = new SensorStatusSeacrh_T01Entity();
                //_pSensorStatusSeacrh_T01Entity.CORP_CODE = _pCORP_CODE;
                _pSensorStatusSeacrh_T01Entity.USER_CODE = _pUSER_CODE;
                _pSensorStatusSeacrh_T01Entity.WINDOW_NAME = _pWINDOW_NAME;
                _pSensorStatusSeacrh_T01Entity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pSensorStatusSeacrh_T01Entity.CRUD = "";
                if (_pFirstYN)
                {
        ...
        private void InitializeControl()
        {
            ...
            {
                //조회조건 영역 
                _luT_DATE.FromDateTime = DateTime.Today;  //조회 시작일, 매월 초 날짜로 설정
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                _pSensorStatusSeacrh_T01Entity.CRUD = "R";
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager));
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                if (tDataTable != null)
                {
                    ...
                    {
                        //체크된것만 저장
                        if (tDataTable.Rows[a]["CRUD"].ToString() == "Y")
                        {
                            sList = sList + "'" + tDataTable.Rows[a]["CD"].ToString() + "'";
                        }
                    }
                }

                _pSensorStatusSeacrh_T01Entity.RESOURCE_CODE = sList.Replace("''", "','").Replace("'", "").ToString();
                _pSensorStatusSeacrh_T01Entity.FROM_DATE = _luT_DATE.FromDateTime.ToString().Substring(0, 10);
                _pSensorStatusSeacrh_T01Entity.TO_DATE = _luT_DATE.ToDateTime.ToString().Substring(0, 10);

                stdate = _luT_DATE.FromDateTime;
                ltdate = _luT_DATE.ToDateTime;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                DataTable _dtMList = new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "SENSOR_DATA_HISTORY", "", "", "").Tables[0];

                if (_pSensorStatusSeacrh_T01Entity.CRUD == "") _dtMList.Rows.Clear();

                if (_dtMList != null && _dtMList.Rows.Count > 0)//&& _dtMList.Rows.Count > 0) )|| (_dtMList != null && _pNoticeRegisterEntity.CRUD == ""))
                {
                    CoFAS_DevExpressManager.BindGridControl(_gdMAIN, _gdMAIN_VIEW, _dtMList); //데이터 필드에 맞춰 자동 바인딩
                }
        ...
        private void Dashboard_DisplayData()
        {
            try
            {
                ...
                _dtList = new SensorStatusSeacrh_T01Business().SensorStatusSeacrh_T01_Info_Filename(_pSensorStatusSeacrh_T01Entity); //Entity에 WINDOW_NAME 추가
                ...
                for (int i = 0; i < _dtList.Rows.Count; i++)
                {
                    ...
                    //    _obList = new SensorStatusSeacrh_T01Business().Templete_Download(_pSensorStatusSeacrh_T01Entity, menu_no, dsp_sort);
                ...
                strFTP_PATH = string.Format(@"{0}{1}/{2}/{3}/", _pFTP_IP_PORT, "USER", "PROGRAM_VIEW", _pSensorStatusSeacrh_T01Entity.WINDOW_NAME);
                ...
                // xml의 parameter 설정
                //dashboard.Parameters[0].Value = _pSensorStatusSeacrh_T01Entity.RESOURCE_CODE;
                dashboard.Parameters[0].Value = _pSensorStatusSeacrh_T01Entity.RESOURCE_CODE;
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
                ...
                isError = new SensorStatusSeacrh_T01Business().Sample_Info_Save(_pSensorStatusSeacrh_T01Entity, dtSave);
