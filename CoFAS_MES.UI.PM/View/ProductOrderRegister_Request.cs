...
{
    public partial class ProductOrderRegister_Request : frmBaseNone
    {
        #region ○ 변수선언
        ...
        private ProductOrderRegister_RequestEntity _pProductOrderRegister_RequestEntity = null; // 엔티티 생성
        ...
        public ProductOrderRegister_Request()
        {
            InitializeComponent();
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pProductOrderRegister_RequestEntity = new ProductOrderRegister_RequestEntity();
                //_pProductOrderRegister_RequestEntity.CORP_CODE = _pCORP_CODE;
                _pProductOrderRegister_RequestEntity.USER_CODE = _pUSER_CODE;
                _pProductOrderRegister_RequestEntity.USER_NAME = _pUSER_NAME;
                _pProductOrderRegister_RequestEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pProductOrderRegister_RequestEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                if (_pFirstYN)
                {
                    _pProductOrderRegister_RequestEntity.CRUD = "";
                    MainFind_DisplayData(_pProductOrderRegister_RequestEntity.CRUD); //Dev Grid 데이터소스 로 바인딩 작업 처리 할 경우에는 최초 빈 데이터 테이블을 바인딩 해야 됨.
        ...
        private void Form_SaveButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                {
                    _pProductOrderRegister_RequestEntity.CRUD = "U";
                }
                else
                {
                    _pProductOrderRegister_RequestEntity.CRUD = "C";
                }
                
                _pProductOrderRegister_RequestEntity.USER_CODE = _pUSER_CODE;
                _pProductOrderRegister_RequestEntity.ORDER_ID = _luORDER_ID.Text;
                //_pMaterialOrderRegisterEntity.ORDER_DATE = _luORDER_DATE.DateTime.ToString().Substring(0,10).Replace("-", "").ToString();
                _pProductOrderRegister_RequestEntity.ORDER_DATE = DateTime.Parse(_luORDER_DATE.DateTime.ToString()).ToString("yyyyMMdd");

                _pProductOrderRegister_RequestEntity.ORDER_SEQ = "";
                _pProductOrderRegister_RequestEntity.VEND_CODE = _luVEND.CodeText.ToString();
                _pProductOrderRegister_RequestEntity.PART_CODE = _luPART.CodeText.ToString();
                _pProductOrderRegister_RequestEntity.ORDER_QTY = _luORDER_QTY.Text;
                //_pMaterialOrderRegisterEntity.REQUEST_DATE = _luREQUEST_DATE.DateTime.ToString().Substring(0, 10).Replace("-", "").ToString();
                _pProductOrderRegister_RequestEntity.REQUEST_DATE = DateTime.Parse(_luREQUEST_DATE.DateTime.ToString()).ToString("yyyyMMdd");
                _pProductOrderRegister_RequestEntity.REQUEST_LOCATION = _luREQUEST_LOCATION.Text;
                _pProductOrderRegister_RequestEntity.UNIT_CODE = _luUNIT_CODE.GetValue();
                _pProductOrderRegister_RequestEntity.UNITCODT_CURRENCY_CODE = _luREQUEST_LOCATION.Text;
                _pProductOrderRegister_RequestEntity.UNITCOST = _luUNITCOST.Text;
                _pProductOrderRegister_RequestEntity.COST = _luCOST.Text;
                _pProductOrderRegister_RequestEntity.REMARK = _luREMARK.Text;
                _pProductOrderRegister_RequestEntity.USE_YN = _luUSE_YN.GetValue();


                InputData_Save(_pProductOrderRegister_RequestEntity);
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager.TargetSite.ToString()));
        ...
        private void Form_DeleteButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                if (_luORDER_ID.Text.ToString() != "")
                {
                    _pProductOrderRegister_RequestEntity.CRUD = "D";
                }
                else
                {
                    MessageBox.Show(_pMSG_CHECK_VALID_ITEM);
        ...
        private void Sheet_Info_Sheet_Data(DataTable tTempData, string pVend_code,string pMIN_Request_Date)   //추가
        {
            try
            {
                ...
                using (DBManager pDBManager = new DBManager())
                {
                    ...
                    _pProductOrderRegister_RequestEntity.VEND_CODE = pVend_code;
                    _pProductOrderRegister_RequestEntity.ORDER_ID = "";
                    _dsList = new ProductOrderRegister_RequestProvider(pDBManager).Sheet_Info_Sheet_Data(_pProductOrderRegister_RequestEntity);
                    //품목정보용
                    _dtList = _dsList.Tables[0];
                    //시험항목검사결과(엑셀바인딩용)
                    _dtList2 = _dsList.Tables[1];
                   // _pProductOrderRegister_RequestEntity.dtListCnt = _dtList_01.Rows.Count;
        ...
        private void SheetFind_DisplayData()
        {
            try
            {
                ...
                _pProductOrderRegister_RequestEntity.CRUD = "R";
                //조회할거 프로시져
                _dtList = new ProductOrderRegister_RequestBusiness().Sheet_Info_Sheet(_pProductOrderRegister_RequestEntity);   //엑셀 시트 조회하기
        ...
        private void MainFind_DisplayData(string pCRUD)
        {
            ...
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
                _pProductOrderRegister_RequestEntity.CRUD = pCRUD;
                _pProductOrderRegister_RequestEntity.DATE_FROM = DateTime.Parse(_luTORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");
                _pProductOrderRegister_RequestEntity.DATE_TO = DateTime.Parse(_luTORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");

                _pProductOrderRegister_RequestEntity.ORDER_ID = _luTORDER_ID.Text;
                _pProductOrderRegister_RequestEntity.USE_YN = _luTUSE_YN.GetValue();
                _pProductOrderRegister_RequestEntity.VEND_CODE = _luTVEND_CODE.CodeText.ToString();
                _pProductOrderRegister_RequestEntity.VEND_NAME = _luTVEND_CODE.NameText.ToString();
                _pProductOrderRegister_RequestEntity.PART_CODE = _luTPART_CODE.CodeText.ToString();
                _pProductOrderRegister_RequestEntity.PART_NAME = _luTPART_CODE.NameText.ToString();
                _pProductOrderRegister_RequestEntity.VEND_PART_CODE = _luTVEND_PART_CODE.Text.ToString();

                using (DBManager pDBManager = new DBManager())
                {

                    _dtList = new ProductOrderRegister_RequestProvider(pDBManager).ProductOrderRegister_Request_R10(_pProductOrderRegister_RequestEntity);
        ...
        private void InputData_Save(ProductOrderRegister_RequestEntity _pProductOrderRegister_RequestEntityt)
        {
            try
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
                ...
                using (DBManager pDBManager = new DBManager())
                {
                    ...
                    if (CoFAS_DevExpressManager.ShowQuestionMessage(_pMSG_SAVE_QUESTION) == DialogResult.Yes)
                    {
                        ...
                        isError = new ProductOrderRegister_RequestBusiness().ProductOrderRegister_Request_Info_Save(_pProductOrderRegister_RequestEntityt);
                        ...
                        else
                        {
                            ...
                            _pLocation_Code = _pProductOrderRegister_RequestEntityt.RTN_KEY;  //저장 위치 
                            MainFind_DisplayData("R");
                        }
                    }
                    else
        ...
        private void _luPART__OnOpenClick(object sender, EventArgs e)
        {
            ...
            // 0 서비스네임 1 공통코드경우 CODE_TYPE 으로구분자 올린다.
            frmCommonPopup.ARRAY = new object[2] { "Product_Code_T01", "" };
        ...
        private void _luVEND__OnOpenClick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                frmCommonPopup.ARRAY = new object[2] { "PRODUCT_TYPE_T01", "" }; //넘기는 파라메터 에 따라 설정
        ...
        private void _ucbtINSPECT_REQUEST_Click(object sender, EventArgs e)
        {
           ...
           // CoFAS_ConvertManager.DataTable_FindCount(_gdMAIN_VIEW.GridControl.DataSource as DataTable, "CRUD ='Y'", "");
            ...
            if (CoFAS_DevExpressManager.ShowQuestionMessage(cnt.ToString()+"건의 시험의뢰를 하시겠습니까?") == DialogResult.Yes)
            {
                _pProductOrderRegister_RequestEntity.CRUD = "U";
                bool isError;
                 isError = new ProductOrderRegister_RequestBusiness().ProductOrderRegister_Request_Info_Check_Save(_pProductOrderRegister_RequestEntity, tDataTable);
                ...
                else
                {
                    ...
                    //CoFAS_DevExpressManager.ShowInformationMessage("저장 처리 되었습니다.");
                    CoFAS_DevExpressManager.ShowInformationMessage(_pMSG_SAVE);
                    _pLocation_Code = _pProductOrderRegister_RequestEntity.RTN_KEY;  //저장 위치 
                    MainFind_DisplayData("R");
                }
            }
        ...
        private void _ucbtBOM_SpendQtyCalc_Click(object sender, EventArgs e)
        {
            ...
            // 0 서비스네임 1 공통코드경우 CODE_TYPE 으로구분자 올린다.
            CoFAS_MES.UI.PM.UserForm.frmCommonPopup.ARRAY = new object[2] { "Vend_Code_I", "" };

            //검색조건 전달 0 코드 1 명칭
            CoFAS_MES.UI.PM.UserForm.frmCommonPopup.ARRAY_CODE = new object[2] { _luTVEND_CODE.CodeText.ToString(), _luTVEND_CODE.NameText.ToString() };

            //ucVendCodeInfoPopup xucVendCodeInfoPopup = new CORE.UserControls.ucVendCodeInfoPopup("PartCodeInfo"); //유저컨트롤러 설정 부분
            CoFAS_MES.UI.PM.UserForm.frmCommonPopup xfrmCommonPopup = new CoFAS_MES.UI.PM.UserForm.frmCommonPopup("BOM_SpendQtyCalc"); //유저컨트롤러 설정 부분
            xfrmCommonPopup.ShowDialog();
