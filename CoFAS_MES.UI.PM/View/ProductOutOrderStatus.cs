...
{
    public partial class ProductOutOrderStatus : frmBaseNone
    {
        ...
        private ProductOutOrderStatusEntity _pProductOutOrderStatusEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pProductOutOrderStatusEntity = new ProductOutOrderStatusEntity();
                //_pProductOutOrderStatusEntity.CORP_CODE = _pCORP_CODE;
                _pProductOutOrderStatusEntity.USER_CODE = _pUSER_CODE;
                _pProductOutOrderStatusEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pProductOutOrderStatusEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pProductOutOrderStatusEntity.CRUD = "";
                if (_pFirstYN)
                {
                    //18.07.02 완료후 풀기
                    //MainFind_DisplayData();
                    //CoFAS_DevExpressManager.ShowInformationMessage("개발 준비중입니다.");
                    Dashboard_DisplayData();
                    //CoFAS_DevExpressManager.ShowInformationMessage("조회 되었습니다.");
        ...
        private void InitializeControl()
        {
            ...
            {
                //조회조건 영역 
                _luTSHIPMENT_DATE.FromDateTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);  //조회 시작일, 매월 초 날짜로 설정
                _luTSHIPMENT_DATE.ToDateTime = DateTime.Today;  //조회 종료일, 당일 설정
                _luTINOUT_ID.Text = "";
                _luTPART.CodeText = "";
                _luTPART.NameText = "";
                _luTVEND.CodeText = "";
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                _pProductOutOrderStatusEntity.CRUD = "R";
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager));
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                _pProductOutOrderStatusEntity.DATE_FROM = DateTime.Parse(_luTSHIPMENT_DATE.FromDateTime.ToString()).ToString("yyyyMMdd"); // Convert.ToString(_luTSHIPMENT_DATE.FromDateTime).Substring(0, 10).Replace("-", "");
                _pProductOutOrderStatusEntity.DATE_TO = DateTime.Parse(_luTSHIPMENT_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");  //Convert.ToString(_luTSHIPMENT_DATE.ToDateTime).Substring(0, 10).Replace("-", "");
                //_pProductOutOrderStatusEntity.VEND_NAME = _luVEND_NAME.Text;
                //_pProductOutOrderStatusEntity.PART_NAME = _luPART_NAME.Text;
                _pProductOutOrderStatusEntity.PART_CODE = _luTINOUT_ID.Text;
        ...
        private void Dashboard_DisplayData()
        {
            try
            {
                ...
                _dtList = new ProductOutOrderStatusBusiness().ProductOutOrderStatus_Info_Filename(_pProductOutOrderStatusEntity); //Entity에 WINDOW_NAME 추가
                ...
                strFTP_PATH = string.Format(@"{0}{1}/{2}/{3}/", _pFTP_IP_PORT, "USER", "PROGRAM_VIEW", _pProductOutOrderStatusEntity.WINDOW_NAME);
                ...
                // xml의 parameter 설정
                dashboard.Parameters[0].Value = _pProductOutOrderStatusEntity.CRUD;
                dashboard.Parameters[1].Value = DateTime.Parse(_luTSHIPMENT_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");
                dashboard.Parameters[2].Value = DateTime.Parse(_luTSHIPMENT_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");

                dashboard.Parameters[3].Value = _luTVEND.CodeText;
                dashboard.Parameters[4].Value = _luTVEND.NameText.ToString();
                dashboard.Parameters[5].Value = _luTPART.CodeText;
                dashboard.Parameters[6].Value = _luTPART.NameText;
                dashboard.Parameters[7].Value = _pLANGUAGE_TYPE;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                //_dtList = new ProductOutOrderStatusBusiness().ProductOutOrderStatus_Info_Filename(_pProductOutOrderStatusEntity);

                //if (_pProductOutOrderStatusEntity.CRUD == "") _dtList.Rows.Clear();


                //if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pProductOutOrderStatusEntity.CRUD == ""))
                //{
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
                ...
                //
                //
                // isError = new ProductOutOrderStatusBusiness().Sample_Info_Save(_pProductOutOrderStatusEntity, dtSave);
                //
                // if (isError)
                // {
                //     //오류 발생.
        ...
        private void _luTVEND__OnOpenClick(object sender, EventArgs e)
        {
            ...
            //검색조건 전달 0 코드 1 명칭
            frmCommonPopup.ARRAY_CODE = new object[2] { _luTVEND.CodeText.ToString(), _luTVEND.NameText.ToString() };
            ...
            if (xfrmCommonPopup.dtReturn.Rows.Count <= 0) return;

            _luTVEND.CodeText = xfrmCommonPopup.dtReturn.Rows[0]["VEND_CODE"].ToString();
