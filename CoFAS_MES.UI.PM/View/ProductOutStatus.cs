...
{
    public partial class ProductOutStatus : frmBaseNone
    {
        ...
        private ProductOutStatusEntity _pProductOutStatusEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pProductOutStatusEntity = new ProductOutStatusEntity();
                _pProductOutStatusEntity.CORP_CODE = _pCORP_CODE;
                _pProductOutStatusEntity.USER_CODE = _pUSER_CODE;
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                _pProductOutStatusEntity.CRUD = "R";
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _pProductOutStatusEntity.DATE_FROM = DateTime.Parse(_luORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");// Convert.ToString(_luORDER_DATE.FromDateTime).Substring(0, 10).Replace("-", ""); //추가 수정
                _pProductOutStatusEntity.DATE_TO = DateTime.Parse(_luORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");// Convert.ToString(_luORDER_DATE.ToDateTime).Substring(0, 10).Replace("-", "");      //추가 수정
                _pProductOutStatusEntity.VEND_NAME = _luVEND_NAME.Text;
                _pProductOutStatusEntity.PART_NAME = _luPART_NAME.Text;

                _dtList = new ProductOutStatusBusiness().Sample_Info(_pProductOutStatusEntity);

                if (_pProductOutStatusEntity.CRUD == "") _dtList.Rows.Clear();


                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pProductOutStatusEntity.CRUD == ""))
                {
        ...
        private void SubFind_DisplayData(string strWINDOW_NAME, string strGRID_NAME)
        {
            try
            {
                ...
                _dtList = new ProductOutStatusBusiness().Sample_Info_Sub(_pProductOutStatusEntity);
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
                ...
                isError = new ProductOutStatusBusiness().Sample_Info_Save(_pProductOutStatusEntity, dtSave);
