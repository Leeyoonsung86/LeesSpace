...
{
    public partial class ProductTraceabilityStatus : frmBaseDesign
    {
        ...
        private string _pXMLFile = "";                  // XML 디자인 파일 명
        private bool _pFirstYN = true;                  // 최초실행여부 최초 설정 에서만 사용

        public ProductTraceabilityStatus()
        {
            InitializeComponent();
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                DataTable dtXMLDesign = new XMLDesignBusiness().XMLDesign_Info(_pWINDOW_NAME, _pLANGUAGE_TYPE);
                ...
                if (dtXMLDesign.Rows.Count == 0)
                {
                    CoFAS_DevExpressManager.ShowInformationMessage(_pMSG_NO_SELECT_FILEFORM);
                    return;
                }

                for (int i = 0; i < dtXMLDesign.Rows.Count; i++)
                {
                    _pXMLFile = dtXMLDesign.Rows[i]["XML_FILE_NAME"].ToString();
                }
        ...
        private void InitializeControl()
        {
            ...
            {
                //조회조건 영역 
                _luPLAN_DATE.FromDateTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);  //조회 시작일, 매월 초 날짜로 설정
                _luPLAN_DATE.ToDateTime = DateTime.Today;  //조회 종료일, 당일 설정
                //데이터 영역
        ...
        private void Dashboard_DisplayData()
        {
            try
            {
                ...
                dashboard.Parameters[1].Value = DateTime.Parse(_luPLAN_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");
                dashboard.Parameters[2].Value = DateTime.Parse(_luPLAN_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");
