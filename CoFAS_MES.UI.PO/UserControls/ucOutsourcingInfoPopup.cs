...
{
    public partial class ucOutsourcingInfoPopup : UserControl
    {
        ...
        private ucOutsourcingInfoPopupEntity _pucOutsourcingInfoPopupEntity = null;
        ...
        public ucOutsourcingInfoPopup(DataTable dt)
        {
            InitializeComponent();
        ...
        private void ucOutsourcingInfoPopup_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void InitializeSetting()
        {
            ...
            _pucOutsourcingInfoPopupEntity = new ucOutsourcingInfoPopupEntity();
            //_pOrderNumberInfoPopup_Entity.VEND_CODE = ARRAY[1].ToString();
            _pucOutsourcingInfoPopupEntity.USER_CODE = USER_CODE.ToString();
            ...
            _pucOutsourcingInfoPopupEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
            //화면 컨트롤러 설정
            InitializeControl();
        ...
        private void _luORDER_NUMBER__OnButtonPressed(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            //    CoFAS_MES.UI.PO.UserForm.frmCommonPopup.ARRAY = new object[2] { "", "" }; //넘기는 파라메터 에 따라 설정, 제품/조회할때 품목명을 던지기
            //    CoFAS_MES.UI.PO.UserForm.frmCommonPopup.ARRAY_CODE = new object[2] { "", "" };
            //    CoFAS_MES.UI.PO.UserForm.frmCommonPopup xfrmCommonPopup = new CoFAS_MES.UI.PO.UserForm.frmCommonPopup("ordernumberinfo_popup"); //유저컨트롤러 설정 부분
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _pucOutsourcingInfoPopupEntity.CRUD = "R";
                _pucOutsourcingInfoPopupEntity.LOT_ID = "";
                _pucOutsourcingInfoPopupEntity.LOT_NO = "";
                _pucOutsourcingInfoPopupEntity.ORDER_NUMBER = _luTORDER_NUMBER.Text.ToString();
                _pucOutsourcingInfoPopupEntity.PART_CODE = "";
                _pucOutsourcingInfoPopupEntity.VEND_PART_CODE = _luTPART_CODE.CodeText.ToString();
                _pucOutsourcingInfoPopupEntity.PART_HIGH = _luTPART_HIGH.Text.ToString();
                _pucOutsourcingInfoPopupEntity.PART_MIDDLE = _luTPART_MIDDLE.Text.ToString();
                _pucOutsourcingInfoPopupEntity.PART_LOW = _luTPART_LOW.Text.ToString();


                _dtList = new ucOutsourcingInfoPopupBusiness().ucOutsourcingInfo_Mst(_pucOutsourcingInfoPopupEntity);

                if (_pucOutsourcingInfoPopupEntity.CRUD == "")
                    _dtList.Rows.Clear();

                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pucOutsourcingInfoPopupEntity.CRUD == ""))
                {
                    //데이터 필드에 맞춰 자동 바인딩
