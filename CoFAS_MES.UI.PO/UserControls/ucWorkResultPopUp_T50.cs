...
{
    public partial class ucWorkResultPopUp_T50 : UserControl
    {
        #region ○ 기본 전역변수 영역
        ...
        private ucWorkResultPopUp_T50_Entity _pucWorkResultPopUp_T50_Entity = null;

        private string qWORK_ORDERID = string.Empty;
        private string qWORK_ORDERSEQ = string.Empty;
        private string qPART_CODE = string.Empty;
        private string qPART_NAME = string.Empty;
        private string qPROCESS_CODE = string.Empty;

        private decimal qorder_qty = 0;
        private decimal qok_qty = 0;
        private decimal qng_qty = 0;
        ...
        private void InitializeControl()
        {
            try
            {
                _luRESULT_GUBN.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "RESULT_GUBN", "", "", "").Tables[0], 0, 0, "", false);
                _luPROCESS_CODE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "COMMON_CODE", "PO05", "", "").Tables[0], 0, 0, "", false);
                _luPROCESS_CODE.ReadOnly = true;

                String ProcessCode = _luPROCESS_CODE.GetValue();
                _luTERMINAL_CODE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "POP_TERMINALE_DAESUNG", ProcessCode, "", "").Tables[0], 0, 0, "", false);
        ...
        private void _ucbtSAVE_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // -->>>
                decimal sum_qty = 0;
                decimal result_qty = Convert.ToDecimal(_luRESULT_QTY.Text.Replace(",", ""));
                sum_qty = qok_qty + qng_qty + result_qty;
                if (sum_qty > qorder_qty)
                {
                    CoFAS_DevExpressManager.ShowInformationMessage("지시수량을 초과하여 등록할 수 없습니다.");
                    return;
                }
                if (pDialogResult == DialogResult.Yes)
                {
                    //_pucWorkResultPopUp_T50_Entity.RESOURCE_CODE = _luPROCESS_CODE.GetValue();
                    _pucWorkResultPopUp_T50_Entity.RESOURCE_CODE = _luTERMINAL_CODE.GetValue();                    
                    _pucWorkResultPopUp_T50_Entity.COLLECTION_DATE = DateTime.Now.ToString("yyyyMMddHHmmss");
                    _pucWorkResultPopUp_T50_Entity.PROPERTY_VALUE = _luWORKORDERID.Text+ qWORK_ORDERSEQ;
                    //_pucWorkResultPopUp_T50_Entity.OPTION_CODE = qWORK_ORDERSEQ;
                    _pucWorkResultPopUp_T50_Entity.OPTION_CODE = "";
                    switch (_luRESULT_GUBN.GetValue().ToString())
                    {
                        case "CD501001":
                            _pucWorkResultPopUp_T50_Entity.CONDITION_CODE = _luRESULT_GUBN.GetValue();
                            _pucWorkResultPopUp_T50_Entity.COLLECTION_VALUE = _luRESULT_QTY.Text.Replace(",", "");
                            //_pucWorkResultPopUp_T50_Entity.CONDITION_CODE = "CD501001";
                            break;
                        case "CD501002":
                            _pucWorkResultPopUp_T50_Entity.CONDITION_CODE = _luBAD_CODE.GetValue();
                            _pucWorkResultPopUp_T50_Entity.COLLECTION_VALUE = _luRESULT_QTY.Text.Replace(",", "");
                            break;
                        case "CD501003":
                            _pucWorkResultPopUp_T50_Entity.CONDITION_CODE = _luSTATUS_CODE.GetValue();
                            ...
                            _pucWorkResultPopUp_T50_Entity.COLLECTION_VALUE = pTimeSpan.TotalSeconds.ToString(); // Math.Floor(pTimeSpan.TotalSeconds).ToString(); // _luSTATUS_TIME.Text.Replace("-", "").Replace(" / ", "").Replace(":", "");
                            break;
                    }
                    ...
                    bool isError = false;
                    isError = new ucWorkResultPopUp_T50Business().ucWorkResultPopUp_T50_Save(_pucWorkResultPopUp_T50_Entity);
                    ...
                    if (!isError)
                    {
                        ...
                        if (qorder_qty <= qok_qty + qng_qty + result_qty)
                        {
                            _pucWorkResultPopUp_T50_Entity.CRUD = "U";
                            _pucWorkResultPopUp_T50_Entity.PRODUCTION_ORDER_ID = _luWORKORDERID.Text;
                            _pucWorkResultPopUp_T50_Entity.PRODUCTION_ORDER_SEQ = qWORK_ORDERSEQ;
                            _pucWorkResultPopUp_T50_Entity.COMPLETE_YN = "Y";
                            //업데이트
                            isError = new ucWorkResultPopUp_T50Business().usWorkResultPopUp_T02_Save_01(_pucWorkResultPopUp_T50_Entity);
                        ...
                        /*
                        //지시수량 == 양품 + 불량 일경우 완료 업데이트
                        if (_luRESULT_GUBN.GetValue().ToString() == "CD501003" && _pucWorkResultPopUp_T50_Entity.CONDITION_CODE == "ES011003")
                        {
                            _pucWorkResultPopUp_T50_Entity.CRUD = "U";
                            _pucWorkResultPopUp_T50_Entity.PRODUCTION_ORDER_ID = _pucWorkResultPopUp_T50_Entity.PROPERTY_VALUE;
                            _pucWorkResultPopUp_T50_Entity.PRODUCTION_ORDER_SEQ = _pucWorkResultPopUp_T50_Entity.PRODUCTION_ORDER_SEQ;
                            _pucWorkResultPopUp_T50_Entity.COMPLETE_YN = "Y";
                            //업데이트
                            isError = new ucWorkResultPopUp_T50Business().usWorkResultPopUp_T02_Save_01(_pucWorkResultPopUp_T50_Entity);
