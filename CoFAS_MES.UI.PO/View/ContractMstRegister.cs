...
{
    public partial class ContractMstRegister : frmBaseNone
    {
        #region ○ 기본 전역변수
        ...
        private ContractMstRegisterEntity _pContractMstRegisterEntity = null; // 엔티티 생성
        ...
        private string _pMSG_SELECT_VEND = string.Empty;     //거래처를 선택하세요.  
        private string _pMSG_SELECT_PART = string.Empty;     //품목을 선택하세요.  
        #endregion

        #region ○ 생성자

        public ContractMstRegister()
        {
            InitializeComponent();
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pContractMstRegisterEntity = new ContractMstRegisterEntity();
                _pContractMstRegisterEntity.USER_CODE = _pUSER_CODE;
                _pContractMstRegisterEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                if (dtMessage != null && dtMessage.Rows[0][0].ToString() != "")
                {
                    ...
                    _pMSG_SELECT_VEND = dtMessage.Rows[0]["MSG_SELECT_VEND"].ToString();
                    _pMSG_SELECT_PART = dtMessage.Rows[0]["MSG_SELECT_PART"].ToString();
                }
                ...
                _pContractMstRegisterEntity.CRUD = "";
                if (_pFirstYN)
                {
        ...
        private void InitializeControl()
        {
            try
            {
                ...
                _luTPART.NameText = "";
                _luTPART.CodeText = "";
                _luTUSE_YN.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R",_pLANGUAGE_TYPE, "common_code", "CD99", "", "").Tables[0], 0, 0, "",false);
               ...
               if(_pFTPIP == "192.168.123.246:8500/")
                {
                    _luVEND.CodeText = "V00001";
                    _luVEND.NameText = "대성엔지니어링";
                }
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                InitializeControl2();
                _pContractMstRegisterEntity.CRUD = "R";
                _pContractMstRegisterEntity.DATE_FROM = DateTime.Parse(_luTCONTRACT_DATE.FromDateTime.ToString()).ToString("yyyyMMdd"); //Convert.ToString(_luTCONTRACT_DATE.FromDateTime).Replace("-", "").Substring(0, 8);
                _pContractMstRegisterEntity.DATE_TO = DateTime.Parse(_luTCONTRACT_DATE.ToDateTime.ToString()).ToString("yyyyMMdd"); //Convert.ToString(_luTCONTRACT_DATE.ToDateTime).Replace("-", "").Substring(0, 8);
                _pContractMstRegisterEntity.VEND_NAME = _luTVEND.NameText;
                _pContractMstRegisterEntity.VEND_CODE = _luTVEND.CodeText;
                _pContractMstRegisterEntity.PART_NAME = _luTPART.NameText;
                _pContractMstRegisterEntity.PART_CODE = _luTPART.CodeText;
                _pContractMstRegisterEntity.USE_YN = _luTUSE_YN.GetValue();
                _pContractMstRegisterEntity.CONTRACT_ID = _luTCONTRACT_ID.Text;
        ...
        private void Form_SaveButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                if(_luPART.CodeText == "")
                {
                    ...
                    CoFAS_DevExpressManager.ShowInformationMessage(_pMSG_SELECT_PART);
                    return;
                }
                ...
                if(_luVEND.CodeText == "")
                {
                    ...
                    CoFAS_DevExpressManager.ShowInformationMessage(_pMSG_SELECT_VEND);
                    return;
                }
                ...
                //확인
                if (_luCONTRACT_ID.Text.ToString() != "") //수정
                {                    
                    try
                    {
                        //생산계획 있는지 조회
                        ...
                        _pContractMstRegisterEntity.CRUD = "R";
                        _pContractMstRegisterEntity.DATE_FROM = "00000101";
                        _pContractMstRegisterEntity.DATE_TO = "99991231";
                        _pContractMstRegisterEntity.PART_NAME = "";
                        _pContractMstRegisterEntity.PART_CODE = "";
                        _pContractMstRegisterEntity.USE_YN = "Y";
                        ...
                        {
                            CoFAS_DevExpressManager.ShowInformationMessage(_pMSG_NOT_MODIFY_CONTRACT);
                        }
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new ContractMstRegisterBusiness().ContractMst_Info(_pContractMstRegisterEntity);

                if (_pContractMstRegisterEntity.CRUD == "")
                    _dtList.Rows.Clear();

                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList !=null && _pContractMstRegisterEntity.CRUD == ""))
                {
                    //데이터 필드에 맞춰 자동 바인딩
        ...
        private string Check_Next_Process()
        {
            try
            {
                ...
                _dtList2 = new ContractMstRegisterBusiness().Sample_Check_Next_Process(_pContractMstRegisterEntity);

                if (_pContractMstRegisterEntity.CRUD == "")
                    _dtList2.Rows.Clear();

                //조회결과 있으면
                if ((_dtList2 != null && _dtList2.Rows.Count > 0) || (_dtList2 != null && _pContractMstRegisterEntity.CRUD == ""))
                {
                    //데이터 필드에 맞춰 자동 바인딩
