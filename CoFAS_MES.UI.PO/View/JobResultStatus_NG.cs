...
{
    public partial class JobResultStatus_NG : frmBaseNone
    {
        ...
        private JobResultStatus_NGEntity _pJobResultStatus_NGEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pJobResultStatus_NGEntity = new JobResultStatus_NGEntity();
                //_pJobResultStatus_NGEntity.CORP_CODE = _pCORP_CODE;
                _pJobResultStatus_NGEntity.USER_CODE = _pUSER_CODE;
                _pJobResultStatus_NGEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pJobResultStatus_NGEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pJobResultStatus_NGEntity.CRUD = "";
                if (_pFirstYN)
                {
        ...
        private void InitializeControl()
        {
            ...
            {
                //조회조건 영역 
                _luT_ORDER_DATE.FromDateTime = DateTime.Today;  //조회 시작일, 매월 초 날짜로 설정
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                _pJobResultStatus_NGEntity.CRUD = "R";
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager));
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                _pJobResultStatus_NGEntity.CRUD = "R";
                _pJobResultStatus_NGEntity.DATE_FROM = Convert.ToString(_luT_ORDER_DATE.FromDateTime).Substring(0, 10).Replace("-", "");
                _pJobResultStatus_NGEntity.DATE_TO = Convert.ToString(_luT_ORDER_DATE.ToDateTime).Substring(0, 10).Replace("-", "");
        ...
        private void Dashboard_DisplayData()
        {
            try
            {
                ...
                _dtList = new JobResultStatus_NGBusiness().JobResultStatus_NG_Info_Filename(_pJobResultStatus_NGEntity); //Entity에 WINDOW_NAME 추가
                ...
                strFTP_PATH = string.Format(@"{0}{1}/{2}/{3}/", _pFTP_IP_PORT, "USER", "PROGRAM_VIEW", _pJobResultStatus_NGEntity.WINDOW_NAME);
               ...
               // { 
                    // xml의 parameter 설정
                    dashboard.Parameters[0].Value = _pJobResultStatus_NGEntity.CRUD;
                    ...
                    dashboard.Parameters[5].Value = _pLANGUAGE_TYPE;
               //}
               //else
               //{ 
               //    dashboard.Parameters[0].Value = _pJobResultStatus_NGEntity.CRUD;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                //_dtList = new JobResultStatus_NGBusiness().JobResultStatus_NG_Info_Filename(_pJobResultStatus_NGEntity);

                //if (_pJobResultStatus_NGEntity.CRUD == "") _dtList.Rows.Clear();


                //if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pJobResultStatus_NGEntity.CRUD == ""))
                //{
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
                ...
                isError = new JobResultStatus_NGBusiness().Sample_Info_Save(_pJobResultStatus_NGEntity, dtSave);
        ...
        private void _luTPART__OnOpenClick(object sender, EventArgs e)
        {
            try
            {
                ...
                frmCommonPopup.ARRAY = new object[2] { "Product_Code_02", _luT_PART.NameText }; //넘기는 파라메터 에 따라 설정, 제품/조회할때 품목명을 던지기
