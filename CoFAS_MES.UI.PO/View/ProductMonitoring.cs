...
{
    public partial class ProductMonitoring : frmBaseNone
    {
        ...
        private System.Threading.Timer tmrRefresh;            //조회 새로고침
                                                              // 자동조회를 위한 변수선언
        private int pMonitorTimer = 0;
        public int pMonitorIndex = 0;

        private ProductMonitoringEntity _pProductMonitoringEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pProductMonitoringEntity = new ProductMonitoringEntity();
                //_pProductMonitoringEntity.CORP_CODE = _pCORP_CODE;
                _pProductMonitoringEntity.USER_CODE = _pUSER_CODE;
                _pProductMonitoringEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pProductMonitoringEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pProductMonitoringEntity.CRUD = "R";
                if (_pFirstYN)
                {
                ...
                pMonitorIndex = Convert.ToInt32(_luTIMER.Text);


                tmrRefresh = new System.Threading.Timer(new TimerCallback(RefreshTime), null, pMonitorIndex * 1000, pMonitorIndex * 1000);
        ...
        private void RefreshTime(object obj)
        {
           ...
           // tmrRefresh.Change(Timeout.Infinite, Timeout.Infinite);
        ...
        private void Dashboard_DisplayData()
        {
            try
            {
                ...
                _dtList = new ProductMonitoringBusiness().ProductMonitoring_Info_Filename(_pProductMonitoringEntity); //Entity에 WINDOW_NAME 추가
                ...
                strFTP_PATH = string.Format(@"{0}{1}/{2}/{3}/", _pFTP_IP_PORT, "USER", "PROGRAM_VIEW", _pProductMonitoringEntity.WINDOW_NAME);
                ...
                //dashboard.LoadFromXml(curfile); //xml 파일 로드
                    ...
                    dashboard.Parameters[0].Value = _pProductMonitoringEntity.CRUD;
                    dashboard.Parameters[1].Value =  DateTime.Now.ToString("yyyyMMdd");//_luT_ORDER_DATE.FromDateTime.ToString().Substring(0, 10).Replace("-", "");
                    dashboard.Parameters[2].Value = DateTime.Now.ToString("yyyyMMdd");//_luT_ORDER_DATE.ToDateTime.ToString().Substring(0, 10).Replace("-", "");
                    dashboard.Parameters[3].Value = "";
                    dashboard.Parameters[4].Value = "";
                    dashboard.Parameters[5].Value = ""; // 공정번호 필요시에 추가.
                    dashboard.Parameters[6].Value = ""; // 공정번호 필요시에 추가.
                    dashboard.Parameters[7].Value = ""; // 공정명 필요시에 추가.
                    dashboard.Parameters[8].Value = _pLANGUAGE_TYPE;
                    dashboard.Parameters[9].Value = "";
                    dashboard.Parameters[10].Value = "";
                ...
                //pMonitorIndex = Convert.ToInt32(_luTIMER.Text);
                //
                //tmrRefresh.Change(pMonitorIndex * 1000, pMonitorIndex * 1000);
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                //_dtList = new ProductMonitoringBusiness().ProductMonitoring_Info_Filename(_pProductMonitoringEntity);

                //if (_pProductMonitoringEntity.CRUD == "") _dtList.Rows.Clear();


                //if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pProductMonitoringEntity.CRUD == ""))
                //{
        ...
        private void _ucbtTIMER_CHANGE_Click(object sender, EventArgs e)
        {
            pMonitorIndex = Convert.ToInt32(_luTIMER.Text);
