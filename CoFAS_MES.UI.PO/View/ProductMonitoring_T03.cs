...
{
    public partial class ProductMonitoring_T03 : frmBaseNone
    {
        ...
        private ProductMonitoring_T03Entity _pProductMonitoring_T03Entity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pProductMonitoring_T03Entity = new ProductMonitoring_T03Entity();
                //_pProductMonitoring_T01Entity.CORP_CODE = _pCORP_CODE;
                _pProductMonitoring_T03Entity.USER_CODE = _pUSER_CODE;
                _pProductMonitoring_T03Entity.WINDOW_NAME = _pWINDOW_NAME;
                _pProductMonitoring_T03Entity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pProductMonitoring_T03Entity.CRUD = "";
                if (_pFirstYN)
                {
        ...
        private void InitializeControl()
        {
            try
            {
                ...
                _lbOrder_Count.Text = "0";
                _lbOrder_qty.Text = "0";
                _lbResult_qty.Text = "0";
                ...
                _pProductMonitoring_T03Entity.CRUD = "R";
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                //_dtList = new ProductMonitoring_T01Business().ProductMonitoring_T01_Info_Filename(_pProductMonitoring_T01Entity);

                //if (_pProductMonitoring_T01Entity.CRUD == "") _dtList.Rows.Clear();


                //if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pProductMonitoring_T01Entity.CRUD == ""))
                //{
                ...
                string sfilename = "";
                _pProductMonitoring_T03Entity.CRUD = "R";
                _pProductMonitoring_T03Entity.DATE_FROM = DateTime.Parse(_luTORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");
                _pProductMonitoring_T03Entity.DATE_TO = DateTime.Parse(_luTORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");
                _pProductMonitoring_T03Entity.VEND_NAME = "";// _luTVEND_NAME.Text;
                

                _dtList = new ProductMonitoring_T03Business().ProductMonitoring_T03_Info(_pProductMonitoring_T03Entity); //Entity에 WINDOW_NAME 추가
                
                if (_pProductMonitoring_T03Entity.CRUD == "")
                    _dtList.Rows.Clear();

                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pProductMonitoring_T03Entity.CRUD == ""))
                {
                    ...
                    double count = Convert.ToDouble(_dtList.Rows.Count.ToString());
                    _lbOrder_Count.Text = string.Format("{0:#,###}", Convert.ToInt32(count.ToString()));

                    double Order_qty = 0;
                    for (int i = 0; i < _dtList.Rows.Count; i++)
                    {
                        Order_qty = Order_qty + Convert.ToDouble(_dtList.Rows[i][6].ToString());
                    }
                    _lbOrder_qty.Text = string.Format("{0:#,###}", Convert.ToInt32(Order_qty.ToString()));

                    double sum2 = 0;
                    for (int i = 0; i < _dtList.Rows.Count; i++)
                    {
                        sum2 = sum2 + Convert.ToDouble(_dtList.Rows[i][8].ToString());
                    }

                    _lbResult_qty.Text = string.Format("{0:#,###}",Convert.ToInt32(sum2.ToString()));
        ...
        private void gdMAIN_VIEW_RowStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowStyleEventArgs e)
        {
            ...
            string Production_order_id = view.GetRowCellDisplayText(e.RowHandle, view.Columns["PRODUCTION_ORDER_ID"]); 
