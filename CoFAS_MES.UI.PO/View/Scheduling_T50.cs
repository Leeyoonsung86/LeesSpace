...
{
    public partial class Scheduling_T50 : frmBaseNone
    {
        ...
        private Scheduling_T50Entity _pScheduling_T50Entity = null; // 엔티티 생성
        ...
        private int tlpCount = 4;
        private int publicMargin = 10;

        private bool _pFirstYN = true;                  // 최초실행여부 최초 설정 에서만 사용


        public Scheduling_T50()
        {
            InitializeComponent();
        ...
        private void Form_Load(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                //_luT_PROCESS_CODE.ValueChanged += _luT_PROCESS_CODE_ValueChanged;
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager));
        ...
        //{
        //    _luT_RESOURCE_CODE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "WORKCENTER_DS", _luT_PROCESS_CODE.GetValue(), "", "").Tables[0], 0, 0, "");
        //}
        #endregion
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //_pScheduling_T50Entity.CORP_CODE = _pCORP_CODE;
                _pScheduling_T50Entity.USER_CODE = _pUSER_CODE;
                _pScheduling_T50Entity.WINDOW_NAME = _pWINDOW_NAME;
                _pScheduling_T50Entity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
        ...
        private void InitializeControl()
        {
            try
            {
                ...
                //_luT_PROCESS_CODE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "PROCESSCODE_MAIN", "", "", "").Tables[0], 0, 0, "");
                //_luT_RESOURCE_CODE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "WORKCENTER_DS", _luT_PROCESS_CODE.GetValue(), "", "").Tables[0], 0, 0, "");
                _luT_DATE.DateTime = DateTime.Today;
        ...
        private void MainFind_DisplayData_LeftGrid()
        {
            
            _dtList = new Scheduling_T50Business().Scheduling_T50_Info(_pScheduling_T50Entity); //Entity에 WINDOW_NAME 추가

            if (_pScheduling_T50Entity.CRUD == "")
                _dtList.Rows.Clear();

            if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pScheduling_T50Entity.CRUD == ""))
            {
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                //_pScheduling_T50Entity.process_code_mst = _luT_PROCESS_CODE.GetValue();
                //_pScheduling_T50Entity.process_code = _luT_RESOURCE_CODE.GetValue();
                _pScheduling_T50Entity.order_date = DateTime.Parse(_luT_DATE.DateTime.ToString()).ToString("yyyyMMdd") == "00010101" ? "" : DateTime.Parse(_luT_DATE.DateTime.ToString()).ToString("yyyyMMdd");
                //_pScheduling_T50Entity.DATE_FROM = DateTime.Parse(_luTORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");
                //_pScheduling_T50Entity.DATE_TO = DateTime.Parse(_luTORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");

                MainFind_DisplayData_LeftGrid();
        ...
        private void SUBFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new Scheduling_T50Business().Scheduling_T50_R13(_pScheduling_T50Entity); //Entity에 WINDOW_NAME 추가
        ...
        void UC_OnPopup(object sender, EventArgs e)
        {
            //MainFind_DisplayData_LeftGrid();
            SUBFind_DisplayData();
