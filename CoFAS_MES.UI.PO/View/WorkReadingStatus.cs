...
{
    public partial class WorkReadingStatus : frmBaseNone
    {
        ...
        private WorkReadingStatusEntity _pWorkReadingStatusEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pWorkReadingStatusEntity = new WorkReadingStatusEntity();
                //_pWorkReadingStatusEntity.CORP_CODE = _pCORP_CODE;
                _pWorkReadingStatusEntity.USER_CODE = _pUSER_CODE;
                _pWorkReadingStatusEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pWorkReadingStatusEntity.CRUD = "";
        ...
        private void InitializeControl(bool bSearch)
        {
            try
            {
                ...
                _luTREADING_TYPE.AddValue(new CommonCodeReturnBusiness().CommonCode_Return("R", _pLANGUAGE_TYPE, "COMMON_CODE", "CD70", "", "").Tables[0], 0, 0, "", true);
                _luTREADING_TYPE.ItemIndex = 0;
                _luTREADING_DATE.FromDateTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);  //조회 시작일, 매월 초 날짜로 설정
                _luTREADING_DATE.ToDateTime = DateTime.Today;  //조회 종료일, 당일 설정
                _luTUSER_ACCOUNT.Text = "";
        ...
        private void _gdMAIN_VIEW_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                _pWorkReadingStatusEntity.CRUD = "R";
            }
            catch (ExceptionManager pExceptionManager)
            {
                CoFAS_DevExpressManager.ShowErrorMessage(string.Format("{0}\n{1}", pExceptionManager.Exception.Message.ToString(), pExceptionManager.TargetSite.ToString()));
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
              ...
              //  InitializeControl(false);
                    ...
                    _pWorkReadingStatusEntity.CRUD = "R";
                    _pWorkReadingStatusEntity.READING_TYPE = _luTREADING_TYPE.GetValue();
                    _pWorkReadingStatusEntity.DATE_FROM = DateTime.Parse(_luTREADING_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");
                    _pWorkReadingStatusEntity.DATE_TO = DateTime.Parse(_luTREADING_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");
                    _pWorkReadingStatusEntity.USER_ACCOUNT = _luTUSER_ACCOUNT.Text;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                if (_pWorkReadingStatusEntity.CRUD == "") _dtList.Rows.Clear();
        ...
        private void InputData_Save(WorkReadingStatusEntity _pWorkReadingStatusEntity)
        {
            try
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
