...
{
    public partial class WorkResultRegister : frmBaseNone
    {
        #region ○ 기본 전역변수
        ...
        private WorkResultRegisterEntity _pWorkResultRegisterEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pWorkResultRegisterEntity = new WorkResultRegisterEntity();
                _pWorkResultRegisterEntity.USER_CODE = _pUSER_CODE;
                _pWorkResultRegisterEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pWorkResultRegisterEntity.CRUD = "";
                if (_pFirstYN)
                {
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                _pWorkResultRegisterEntity.CRUD = "R";
                _pWorkResultRegisterEntity.DATE_FROM = DateTime.Parse(_luTORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");
                _pWorkResultRegisterEntity.DATE_TO = DateTime.Parse(_luTORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");
                _pWorkResultRegisterEntity.PART_CODE = _luTPART.CodeText;
                _pWorkResultRegisterEntity.PART_NAME = _luTPART.NameText;
                _pWorkResultRegisterEntity.PRODUCTION_ORDER_ID = _luTWORK_ORDER.Text;
                _pWorkResultRegisterEntity.PROCESS_CODE_MST = _luTPC_CODE.GetValue();
                _pWorkResultRegisterEntity.PROCESS_CODE = _luTWC_CODE.GetValue();
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new WorkResultRegisterBusiness().WorkResultRegister_Info_Mst(_pWorkResultRegisterEntity);

                if (_pWorkResultRegisterEntity.CRUD == "")
                    _dtList.Rows.Clear();

                if((_dtList != null && _dtList.Rows.Count > 0) || (_dtList !=null && _pWorkResultRegisterEntity.CRUD == ""))
                {
                    //데이터 필드에 맞춰 자동 바인딩
        ...
        private void SubFind_DisplayData(string qWorkOrder, string qColumnHeader)
        {
            try
            {
                ...
                _pWorkResultRegisterEntity.PRODUCTION_ORDER_ID = qWorkOrder;
                _pWorkResultRegisterEntity.CODE_GUBN = qColumnHeader;
                _dtSub = new WorkResultRegisterBusiness().WorkResultRegister_Info_Sub1(_pWorkResultRegisterEntity);
        ...
        private void InputData_Save(DataTable dtSave) //(WorkResultRegisterEntity _pWorkResultRegisterEntity)
        {
            try
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
                ...
                isError = new WorkResultRegisterBusiness().WorkResultMst_Save(_pWorkResultRegisterEntity,dtSave);
                if (!isError)
                {                   
                    //정상 처리
