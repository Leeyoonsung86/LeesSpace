...
{
    public partial class WorkResultRegister_T50 : frmBaseNone
    {
        #region ○ 기본 전역변수
        ...
        private string qEquipment_code = string.Empty;     // 설비코드 체크
        private decimal order_qty = 0;
        private decimal ok_qty = 0;
        private decimal ng_qty = 0;
        private WorkResultRegister_T50Entity _pWorkResultRegister_T50Entity = null; // 엔티티 생성
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                _pWorkResultRegister_T50Entity.CRUD = "R";
                _pWorkResultRegister_T50Entity.DATE_FROM = DateTime.Parse(_luTORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");
                _pWorkResultRegister_T50Entity.DATE_TO = DateTime.Parse(_luTORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");
                _pWorkResultRegister_T50Entity.PART_CODE = _luTPART.CodeText;
                _pWorkResultRegister_T50Entity.PART_NAME = _luTPART.NameText;
                _pWorkResultRegister_T50Entity.PRODUCTION_ORDER_ID = _luTWORK_ORDER.Text;
                _pWorkResultRegister_T50Entity.PROCESS_CODE_MST = ""; // _luTPC_CODE.GetValue();
                _pWorkResultRegister_T50Entity.PROCESS_CODE = _luTWC_CODE.GetValue();
        ...
        private void Form_AddItemButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                if(qEquipment_code == "")
                {
                    CoFAS_DevExpressManager.ShowInformationMessage("설비 지정 후 가능합니다.");
                    return;
                }
                ...
                {
                    // 작업장 필수항목 체크 - 팝업 불필요
                    if (_pWorkResultRegister_T50Entity.PROCESS_CODE == "")
                    {
                        CoFAS_DevExpressManager.ShowInformationMessage(_pMSG_PROCESS_CODE);
                        return;
                    }
                    // 작업상태 완료시 체크 - 수정불가
                    if (_pWorkResultRegister_T50Entity.USE_YN == "ES011003")
                    {
                    ...
                    _pLocation_Code = _pWorkResultRegister_T50Entity.PRODUCTION_ORDER_ID;
                    
                    UI.PO.UserForm.frmCommonPopup.ARRAY_CODE = new object[8] { _pWorkResultRegister_T50Entity.PRODUCTION_ORDER_ID, _pWorkResultRegister_T50Entity.PART_CODE, _pWorkResultRegister_T50Entity.PART_NAME, _pWorkResultRegister_T50Entity.PROCESS_CODE, _pWorkResultRegister_T50Entity.PRODUCTION_ORDER_SEQ, order_qty,ok_qty,ng_qty };
                    UI.PO.UserForm.frmCommonPopup xfrmCommonPopup = new UI.PO.UserForm.frmCommonPopup("WorkResult_T50"); //유저컨트롤러 설정 부분
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new WorkResultRegister_T50Business().WorkResultRegister_T50_Info_Mst(_pWorkResultRegister_T50Entity);

                if (_pWorkResultRegister_T50Entity.CRUD == "")
                    _dtList.Rows.Clear();

                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pWorkResultRegister_T50Entity.CRUD == ""))
                {
                    //데이터 필드에 맞춰 자동 바인딩
        ...
        private void SubFind_DisplayData(string qWorkOrder,string qWorkOrderSeq, string qColumnHeader)
        {
            try
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);

                _pWorkResultRegister_T50Entity.PRODUCTION_ORDER_ID = qWorkOrder;
                _pWorkResultRegister_T50Entity.PRODUCTION_ORDER_SEQ = qWorkOrderSeq;
                _pWorkResultRegister_T50Entity.CODE_GUBN = qColumnHeader;
                _dtSub = new WorkResultRegister_T50Business().WorkResultRegister_T50_Info_Sub1(_pWorkResultRegister_T50Entity);
