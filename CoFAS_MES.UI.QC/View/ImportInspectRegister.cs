...
{
    public partial class ImportInspectRegister : frmBaseNone
    {
        #region ○ 기본 전역변수
        ...
        private ImportInspectRegisterEntity _pImportInspectRegisterEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pImportInspectRegisterEntity = new ImportInspectRegisterEntity();
                _pImportInspectRegisterEntity.USER_CODE = _pUSER_CODE;
                _pImportInspectRegisterEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
                ...
                _pImportInspectRegisterEntity.CRUD = "";
                if (_pFirstYN)
                {
        ...
        private void InitializeControl()
        {
            try
            {
                ...
                _luMI_MEASURES.Text = "0";
                _luMI_MEASURES.ReadOnly = false;

                _luMOISTURE_MEASURES.Text = "0";
                _luMOISTURE_MEASURES.ReadOnly = false;
                ...
                _luINSPECT_DATE.DateTime = DateTime.Today;
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                InitializeControl2();
                _pImportInspectRegisterEntity.CRUD = "R";
                _pImportInspectRegisterEntity.DATE_FROM = DateTime.Parse(_luTINSPECT_DATE.FromDateTime.ToString()).ToString("yyyyMMdd"); //Convert.ToString(_luTCONTRACT_DATE.FromDateTime).Replace("-", "").Substring(0, 8);
                _pImportInspectRegisterEntity.DATE_TO = DateTime.Parse(_luTINSPECT_DATE.ToDateTime.ToString()).ToString("yyyyMMdd"); //Convert.ToString(_luTCONTRACT_DATE.ToDateTime).Replace("-", "").Substring(0, 8);
                _pImportInspectRegisterEntity.ORDER_ID = _luTORDER_ID.Text;
                _pImportInspectRegisterEntity.LOT_NO = _luTLOT_NO.Text;
                _pImportInspectRegisterEntity.USE_YN = _luTUSE_YN.GetValue();
                _pImportInspectRegisterEntity.INSPECT_ID = _luINSPECT_ID.Text;
        ...
        private void Form_SaveButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                if (_luORDER_ID.Text == "")
                {
                    //CoFAS_DevExpressManager.ShowInformationMessage("필수항목을 확인해주세요.");
                ...
                _pImportInspectRegisterEntity.ORDER_ID = _luORDER_ID.Text;
                _pImportInspectRegisterEntity.INSPECT_ID = _luINSPECT_ID.Text;
                _pImportInspectRegisterEntity.INSPECT_DATE = _luINSPECT_DATE.DateTime.ToString("yyyyMMdd");                
                _pImportInspectRegisterEntity.PART_NAME = _luPART_NAME.Text;
                _pImportInspectRegisterEntity.LOT_NO = _luLOT_NO.Text;
                _pImportInspectRegisterEntity.MI_MEASURES = _luMI_MEASURES.Text == "" ? "0.00" : _luMI_MEASURES.Text.Replace(",", "");
                _pImportInspectRegisterEntity.MOISTURE_MEASERES = _luMOISTURE_MEASURES.Text == "" ? "0.00" : _luMOISTURE_MEASURES.Text.Replace(",", "");
                _pImportInspectRegisterEntity.USER_CODE = _pUSER_CODE;
                _pImportInspectRegisterEntity.USE_YN = _luUSE_YN.GetValue();

                InputData_Save(_pImportInspectRegisterEntity);
            }
            //    }
            //}
            catch (ExceptionManager pExceptionManager)
        ...
        private void Form_DeleteButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                //확인
                if (_luPART_NAME.Text != "")
                    _pImportInspectRegisterEntity.CRUD = "D";
                else
                { 
                ...
                _pImportInspectRegisterEntity.MI_MEASURES = _luMI_MEASURES.Text == "" ? "0.00" : _luMI_MEASURES.Text.Replace(",", "");
                _pImportInspectRegisterEntity.MOISTURE_MEASERES = _luMOISTURE_MEASURES.Text == "" ? "0.00" : _luMOISTURE_MEASURES.Text.Replace(",", "");
                _pImportInspectRegisterEntity.USE_YN = "N";
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new ImportInspectRegisterBusiness()._pImportInspectRegister_Info(_pImportInspectRegisterEntity);

                if (_pImportInspectRegisterEntity.CRUD == "")
                    _dtList.Rows.Clear();

                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList !=null && _pImportInspectRegisterEntity.CRUD == ""))
                {
                    //데이터 필드에 맞춰 자동 바인딩
        ...
        private void InputData_Save(ImportInspectRegisterEntity _pImportInspectRegisterEntity)
        {
            try
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
                ...
                bool isError = false;
                isError = new ImportInspectRegisterBusiness().ImportInspect_Info_Save(_pImportInspectRegisterEntity);
                if (!isError)
                {
                    //CoFAS_DevExpressManager.ShowInformationMessage("저장 처리 되었습니다.");
                    CoFAS_DevExpressManager.ShowInformationMessage(_pMSG_SAVE);
                    _pLocation_Code = _pImportInspectRegisterEntity.RTN_KEY;  //저장 위치
                    InitializeControl2();
                    Form_SearchButtonClicked(null, null);
                }
                else
        ...
        #region ○ 수주정보 수정 시, 해당 수주번호의 생산계획 있는지 조회 - Check_Next_Process()
        ...
        private void _luORDER_ID__OnButtonPressed(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                CoFAS_MES.UI.QC.UserForm.frmCommonPopup.ARRAY = new object[2] { "MaterialOrder", "" }; //넘기는 파라메터 에 따라 설정, 제품/조회할때 품목명을 던지기
                CoFAS_MES.UI.QC.UserForm.frmCommonPopup.ARRAY_CODE = new object[2] { _luORDER_ID.Text, "" };              
        ...
        private void _luTextEdit__OnTextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                pMI = Convert.ToDecimal(_luMI_MEASURES.Text == "" ? "0.00" : _luMI_MEASURES.Text);
                pMoisture = Convert.ToDecimal(_luMOISTURE_MEASURES.Text == "" ? "0.00" : _luMOISTURE_MEASURES.Text);
