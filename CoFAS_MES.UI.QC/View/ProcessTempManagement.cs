...
{
    public partial class ProcessTempManagement : frmBaseNone
    {
        ...
        private ProcessTempManagementEntity _pProcessTempManagementEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pProcessTempManagementEntity = new ProcessTempManagementEntity();
                //_pProcessTempManagementEntity.CORP_CODE = _pCORP_CODE;
                _pProcessTempManagementEntity.USER_CODE = _pUSER_CODE;
                _pProcessTempManagementEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pProcessTempManagementEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
               ...
               {
                    _lciT_PROCESS_CODE.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Never;
               }
                //그리드 설정
                InitializeGrid();
                ...
                _pProcessTempManagementEntity.CRUD = "";
                if (_pFirstYN)
                {
        ...
        private void _luT_PROCESS_OnOpenClick(object sender, EventArgs e)
        {
            try
            {
                ...
                frmCommonPopup.ARRAY_CODE = new object[2] { _luT_PROCESS.CodeText.ToString(), _luT_PROCESS.NameText.ToString() };
                frmCommonPopup xfrmCommonPopup = new CORE.UserForm.frmCommonPopup("CommonCode"); //유저컨트롤러 설정 부분
        ...
        private void Dashboard_DisplayData()
        {
            try
            {
                ...
                _dtList = new ProcessTempManagementBusiness().ProcessTempManagement_Info_Filename(_pProcessTempManagementEntity); //Entity에 WINDOW_NAME 추가
                ...
                strFTP_PATH = string.Format(@"{0}{1}/{2}/{3}/", _pFTP_IP_PORT, "USER", "PROGRAM_VIEW", _pProcessTempManagementEntity.WINDOW_NAME);
                ...
                {
                    // xml의 parameter 설정
                    dashboard.Parameters[0].Value = _pProcessTempManagementEntity.CRUD;
                    dashboard.Parameters[1].Value = DateTime.Parse(_luT_REG_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");//_luT_ORDER_DATE.FromDateTime.ToString().Substring(0, 10).Replace("-", "");
                    dashboard.Parameters[2].Value = DateTime.Parse(_luT_REG_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");//_luT_ORDER_DATE.ToDateTime.ToString().Substring(0, 10).Replace("-", "");
                }
                //바이오세라일경우
                ...
                if (DBManager.InitDatabaseName == "coever_mes_hwt")
                {
                    // xml의 parameter 설정
                    dashboard.Parameters[0].Value = _pProcessTempManagementEntity.CRUD;
                    dashboard.Parameters[1].Value = DateTime.Parse(_luT_REG_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");//_luT_ORDER_DATE.FromDateTime.ToString().Substring(0, 10).Replace("-", "");
                ...
                else 
                {
                    // xml의 parameter 설정
                    dashboard.Parameters[0].Value = _pProcessTempManagementEntity.CRUD;
                    dashboard.Parameters[1].Value = _luT_PROCESS.CodeText;
                    dashboard.Parameters[2].Value = _luT_PROCESS.NameText;
                    dashboard.Parameters[3].Value = DateTime.Parse(_luT_REG_DATE.FromDateTime.ToString()).ToString("yyyyMMdd");//_luT_ORDER_DATE.FromDateTime.ToString().Substring(0, 10).Replace("-", "");
                    dashboard.Parameters[4].Value = DateTime.Parse(_luT_REG_DATE.ToDateTime.ToString()).ToString("yyyyMMdd");//_luT_ORDER_DATE.ToDateTime.ToString().Substring(0, 10).Replace("-", "");
                }
        ...
        private void Form_InitialButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                _luT_PROCESS.CodeText = "";
                _luT_PROCESS.NameText = "";
                //_luT_PROCESS_CODE.ReadOnly = false;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                //_dtList = new ProcessTempManagementBusiness().ProcessTempManagement_Info_Filename(_pProcessTempManagementEntity);

                //if (_pProcessTempManagementEntity.CRUD == "") _dtList.Rows.Clear();


                //if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pProcessTempManagementEntity.CRUD == ""))
                //{
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
                ...
                isError = new ProcessTempManagementBusiness().Sample_Info_Save(_pProcessTempManagementEntity, dtSave);
