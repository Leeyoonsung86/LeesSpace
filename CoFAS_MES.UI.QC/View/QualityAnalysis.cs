...
{
    public partial class QualityAnalysis : frmBaseNone
    {
        ...
        private QualityAnalsisEntity _pQualityAnalsisEntity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pQualityAnalsisEntity = new QualityAnalsisEntity();
                //_pProductMonitoring_T01Entity.CORP_CODE = _pCORP_CODE;
                _pQualityAnalsisEntity.USER_CODE = _pUSER_CODE;
                _pQualityAnalsisEntity.WINDOW_NAME = _pWINDOW_NAME;
                _pQualityAnalsisEntity.LANGUAGE_TYPE = _pLANGUAGE_TYPE;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                string _terminal_name = _luTTERMINAL_LIST.GetValue();
                //string _check_list = _luTCHECK_CATEGORY.Text;

                DateTime dtF = _luTORDER_DATE.FromDateTime.AddHours(8);
                DateTime dtT = _luTORDER_DATE.ToDateTime.AddHours(8);
                ...
                _pQualityAnalsisEntity.CRUD = "R";
                _pQualityAnalsisEntity.DATE_FROM = dtF.ToString("yyyy-MM-dd hh:mm:ss");
                _pQualityAnalsisEntity.DATE_TO = dtT.ToString("yyyy-MM-dd hh:mm:ss");
                _pQualityAnalsisEntity.TERMINAL_NAME = _terminal_name;
                //_pQualityAnalsisEntity.CHECK_LIST = _check_list.ToString();



                _dtList = new QualityAnalysisBusiness().QualityAnalysis_Mst(_pQualityAnalsisEntity); //Entity에 WINDOW_NAME 추가

                if (_pQualityAnalsisEntity.CRUD == "")
                    _dtList.Rows.Clear();

                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pQualityAnalsisEntity.CRUD == ""))
                {
        ...
        private void gdMAIN_VIEW_RowStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowStyleEventArgs e)
        {
            ...
            string CHECK_DATE = view.GetRowCellDisplayText(e.RowHandle, view.Columns["SPC_DATETIME"]); 
            if (CHECK_DATE == "")
            {
