...
{
    public partial class MonitoringDesigner : frmBaseNone
    {
        ...
        private string _pTEMPLETE_NAME = string.Empty;   // xml 파일 이름 
        ...
        //알림창메시지 복사 끝
        private MonitoringDesignerEntity _pMonitoringDesignerEntity = null; // 엔티티 생성

        private DataTable _dtList = null; //3조회 데이터 리스트
        private object _obList = null;

        private bool _pFirstYN = true;                  // 최초실행여부 최초 설정 에서만 사용

        public MonitoringDesigner()
        {
            InitializeComponent();
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new MonitoringDesignerBusiness().MonitoringDesigner_Info(_pMonitoringDesignerEntity);

                if (_pMonitoringDesignerEntity.CRUD == "") _dtList.Rows.Clear();


                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList != null && _pMonitoringDesignerEntity.CRUD == ""))
                {
        ...
        private void Dashboard_DisplayData()
        {
            try
            {
                ...
                _dtList = new MonitoringDesignerBusiness().MonitoringDesigner_Info_Filename(_pMonitoringDesignerEntity); //Entity에 WINDOW_NAME 추가
                ...
                for (int i = 0; i < _dtList.Rows.Count; i++)
                {
                    ...
                    //    _obList = new SampleRegisterBusiness().Templete_Download(_pSampleRegisterEntity, menu_no, dsp_sort);
                    ...
                    ////// xml의 parameter 설정
                    ////dashboard.Parameters[0].Value = ucTextEdit1.Text.ToString();
                    ////dashboard.Parameters[1].Value = ucTextEdit2.Text.ToString();
                    ////dashboard.Parameters[2].Value = ucTextEdit3.Text.ToString();
                    ////dashboard.Parameters[3].Value = ucTextEdit4.Text.ToString();
                    ////dashboard.Parameters[4].Value = ucTextEdit5.Text.ToString();
        ...
        private void InputData_Save(DataTable dtSave)
        {
            try
            {
                ...
                isError = new MonitoringDesignerBusiness().MonitoringDesigner_Info_Save(_pMonitoringDesignerEntity, dtSave);
                ...
                else
                {
                    ...
                    // 화면 갱신
                    _pMonitoringDesignerEntity.CRUD = "R";
