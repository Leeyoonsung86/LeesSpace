...
{
    public partial class ucUserAuthorityCopy : UserControl
    {
        ...
        private void ucUserAuthorityCopy_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void SubFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new MenuAuthority_T02Business().MenuAuthority_T02_R30(_pMenuAuthority_T02Entity);
        ...
        private void InputDataMain_Save(DataTable dtSave)
        {
            try
            {
                ...
                isError = new MenuAuthority_T02Business().MenuAuthority_T02_I20(_pMenuAuthority_T02Entity, dtSave);
                ...
                {
                    CoFAS_DevExpressManager.ShowInformationMessage("권한복사를 하였습니다.");
                    _pMenuAuthority_T02Entity.COPY_USER_ID = "";
                    MainFind_DisplayData();
                }
        ...
        private void _btSAVE_Click(object sender, EventArgs e)
        {
            try
            {
               ...
               for(int i=0;i< _gdMAIN_VIEW.RowCount;i++)
                {
                     if(_gdMAIN_VIEW.GetRowCellDisplayText(i,"CRUD").ToString()== "Checked")
                    {
                        find_cnt++;
                    }
                }
               if(find_cnt==0)
                {
                    CoFAS_DevExpressManager.ShowInformationMessage("권한을 복사할 사용자를 하나 이상 선택하세요.");
                    return;
                }

               //복사할 사용자와 가져올 사용자 중복 선택 불가
                for (int i = 0; i < _gdMAIN_VIEW.RowCount; i++)
                {
                    if (_gdMAIN_VIEW.GetRowCellDisplayText(i, "CRUD").ToString() == "Checked")
                    {
                        if (_gdMAIN_VIEW.GetRowCellDisplayText(i, "USER_ACCOUNT").ToString() == _gdSUB_VIEW.GetFocusedRowCellValue("USER_ACCOUNT").ToString())
                        {
                            CoFAS_DevExpressManager.ShowInformationMessage("권한을 복사할 사용자와 가져올 사용자를 동시에 선택불가합니다.");
                            return;
                        }
                    }
                ...
                if (CoFAS_DevExpressManager.ShowQuestionMessage(_pMSG_SAVE_QUESTION) == DialogResult.Yes)
                {
                    //_pMenuAuthority_T02Entity.PART_CODE = _luPART_CODE.Text.ToString();
                    //_pMenuAuthority_T02Entity.PART_REVISION_NO = _luPART_REVISION_NO.Text.ToString();
                    _pMenuAuthority_T02Entity.COPY_USER_ID = _gdSUB_VIEW.GetFocusedRowCellValue("USER_ACCOUNT").ToString();
