...
{
    public partial class WorkResultView2 : frmBaseNone
    {
        #region ○ 기본 전역변수
        ...
        private WorkResultView2Entity _pWorkResultView2Entity = null; // 엔티티 생성
        ...
        private void InitializeSetting()
        {
            try
            {
                ...
                //메뉴 화면 엔티티 설정
                _pWorkResultView2Entity = new WorkResultView2Entity();
                _pWorkResultView2Entity.USER_CODE = _pUSER_CODE;
        ...
        private void InitializeControl()
        {
            try
            {
                ...
                //조회조건 영역
                _luTWORK_ORDER_DATE.FromDateTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);  //조회 시작일, 매월 초 날짜로 설정
                _luTWORK_ORDER_DATE.ToDateTime = DateTime.Today;  //조회 종료일, 당일 설정
...
*/
        ...
        private void Form_SearchButtonClicked(object pSender, EventArgs pEventArgs)
        {
            try
            {
                ...
                InitializeControl2();
                _pWorkResultView2Entity.CRUD = "R";
                _pWorkResultView2Entity.DATE_FROM = DateTime.Parse(_luTWORK_ORDER_DATE.FromDateTime.ToString()).ToString("yyyyMMdd"); //Convert.ToString(_luTCONTRACT_DATE.FromDateTime).Replace("-", "").Substring(0, 8);
                _pWorkResultView2Entity.DATE_TO = DateTime.Parse(_luTWORK_ORDER_DATE.ToDateTime.ToString()).ToString("yyyyMMdd"); //Convert.ToString(_luTCONTRACT_DATE.ToDateTime).Replace("-", "").Substring(0, 8);
                //_pWorkResultView2Entity.VEND_NAME = _luTVEND.NameText;
                //_pWorkResultView2Entity.VEND_CODE = _luTVEND.CodeText;
                //_pWorkResultView2Entity.PART_NAME = _luTPART.NameText;
                //_pWorkResultView2Entity.PART_CODE = _luTPART.CodeText;
                //_pWorkResultView2Entity.USE_YN = _luTUSE_YN.GetValue();
                //_pWorkResultView2Entity.CONTRACT_ID = _luTCONTRACT_ID.Text;
        ...
        private void MainFind_DisplayData()
        {
            try
            {
                ...
                _dtList = new WorkResultView2Business().ContractMst_Info(_pWorkResultView2Entity);

                if (_pWorkResultView2Entity.CRUD == "")
                    _dtList.Rows.Clear();

                if ((_dtList != null && _dtList.Rows.Count > 0) || (_dtList !=null && _pWorkResultView2Entity.CRUD == ""))
                {
                    //데이터 필드에 맞춰 자동 바인딩
                    ...
                    {
                        int rowHandle = _gdMAIN_VIEW.LocateByValue("PRODUCTION_ORDER_DATE", _pLocation_Code);
        ...
        private void InputData_Save(WorkResultView2Entity _pWorkResultView2Entity)
        {
            try
            {
                CoFAS_DevExpressManager.SetCursor(this, Cursors.WaitCursor);
                ...
                bool isError = false;
                isError = new WorkResultView2Business().ContractMst_Save(_pWorkResultView2Entity);
                if (!isError)
                {
                    //CoFAS_DevExpressManager.ShowInformationMessage("저장 처리 되었습니다.");
                    CoFAS_DevExpressManager.ShowInformationMessage(_pMSG_SAVE);
                    _pLocation_Code = _pWorkResultView2Entity.RTN_KEY;  //저장 위치
                    InitializeControl2();
